...
"""
build_15m_features.py
=====================
Build 15m feature matrix from BTC/USDT 15m candles using the unified
...
# ============================================================
print("=" * 70)
print("BUILD 15m FEATURE MATRIX (unified pipeline)")
print("=" * 70)

# --- 15m BTC candles (primary OHLCV) ---
print(f"\n{elapsed()} Loading 15m BTC/USDT candles...")
...
print(f"  15m candles: {len(btc)} ({btc.index.min()} to {btc.index.max()})")
...
for frame, name in [(btc_1h, '1H'), (btc_4h, '4H'), (btc_1d, '1D'), (btc_1w, '1W')]:
...
# ============================================================
print("=" * 70)
print("Computing 15m features via feature_library.build_all_features()...")
print("=" * 70)
...
print(f"{elapsed()} Calling build_all_features(tf_name='15m', mode='backfill')...")
...
# ============================================================
# SAVE TO features_15m.db
# ============================================================
print(f"\n{elapsed()} Saving to features_15m.db...")
...
out_db = f'{DB_DIR}/features_15m.db'
# Save to parquet (primary — no column limit)
parquet_path = f'{DB_DIR}/features_15m.parquet'
...
try:
    ...
    df_save.to_sql('features_15m', conn_out, if_exists='replace', index=False)
    conn_out.execute("CREATE INDEX IF NOT EXISTS idx_15m_ts ON features_15m(timestamp)")
    conn_out.close()
except Exception as e:
...
print(f"\n{'='*70}")
print(f"15m FEATURE MATRIX COMPLETE")
print(f"{'='*70}")
