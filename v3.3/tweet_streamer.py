...
def compute_gematria_sentiment(text):
    ...
    # Pass raw text — cipher functions internally filter to ASCII alpha chars
    raw_text = text or ''
    gem_simple = ordinal(raw_text) if raw_text else 0
    gem_eng = english(raw_text) if raw_text else 0
