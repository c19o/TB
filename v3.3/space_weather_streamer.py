...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(DB_DIR, 'space_weather_streamer.log'), encoding='utf-8'),
...
DB_PATH = os.path.join(DB_DIR, 'space_weather.db')
...
def init_db():
    ...
    conn.execute("""
        CREATE TABLE IF NOT EXISTS solar_flares (
            ...
            max_xrlong REAL,
            ts_unix INTEGER,
            flare_gem_ordinal INTEGER,
            flare_gem_dr INTEGER,
            inserted_at TEXT DEFAULT CURRENT_TIMESTAMP,
            UNIQUE(begin_time, max_class)
        )
    """)
    _ensure_sw_cols(conn)
    conn.commit()
    return conn


_SW_NEW_COLS = {
    "kp_dr": "INTEGER", "est_kp_dr": "INTEGER",
    "sw_speed_dr": "INTEGER", "sw_density_dr": "INTEGER",
    "sw_bz_dr": "INTEGER", "sw_bt_dr": "INTEGER",
    "r_scale_dr": "INTEGER", "s_scale_dr": "INTEGER", "g_scale_dr": "INTEGER",
}
_FLARE_NEW_COLS = {"flare_gem_ordinal": "INTEGER", "flare_gem_dr": "INTEGER"}


def _ensure_sw_cols(conn):
    """Add new esoteric columns to existing DBs if not present."""
    existing_sw = {r[1] for r in conn.execute("PRAGMA table_info(space_weather)").fetchall()}
    for col, td in _SW_NEW_COLS.items():
        if col not in existing_sw:
            conn.execute(f"ALTER TABLE space_weather ADD COLUMN {col} {td}")
    existing_fl = {r[1] for r in conn.execute("PRAGMA table_info(solar_flares)").fetchall()}
    for col, td in _FLARE_NEW_COLS.items():
        if col not in existing_fl:
            conn.execute(f"ALTER TABLE solar_flares ADD COLUMN {col} {td}")


def _dr(v):
    """Digital root of a numeric value. Returns None if value is None."""
    if v is None:
        return None
    iv = int(abs(v))
    if iv == 0:
...
def fetch_solar_flares():
    ...
    for entry in data:
        ...
        # Gematria on flare class string (e.g. "M5.2", "X1.1") — letters only
        cls_str = str(max_class).strip()
        gem = _gematria(cls_str)
        flares.append({
            ...
            "ts_unix": ts_unix,
            "flare_gem_ordinal": gem["ordinal"],
            "flare_gem_dr": gem["dr_ordinal"],
        })
    return flares
...
def poll_once(conn):
    ...
    # Compute digital roots for all numeric space weather values
    kp_dr_val     = _dr(kp)
    est_kp_dr_val = _dr(est_kp)
    sp_dr_val     = _dr(sw_speed)
    sd_dr_val     = _dr(sw_density)
    bz_dr_val     = _dr(sw_bz)
    bt_dr_val     = _dr(sw_bt)
    r_dr_val      = _dr(r_scale)
    s_dr_val      = _dr(s_scale)
    g_dr_val      = _dr(g_scale)
    ...
    try:
        conn.execute("""
            ...
            (timestamp, kp_index, estimated_kp, solar_wind_speed,
             ...
             kp_dr, est_kp_dr, sw_speed_dr, sw_density_dr,
             sw_bz_dr, sw_bt_dr, r_scale_dr, s_scale_dr, g_scale_dr)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            now_unix, kp, est_kp, sw_speed,
            sw_density, sw_temp, sw_bz,
            sw_bt, r_scale, s_scale, g_scale,
            kp_dr_val, est_kp_dr_val, sp_dr_val, sd_dr_val,
            bz_dr_val, bt_dr_val, r_dr_val, s_dr_val, g_dr_val,
        ))
        ...
        try:
            conn.execute("""
                ...
                (begin_time, max_time, end_time, max_class, max_xrlong, ts_unix,
                 flare_gem_ordinal, flare_gem_dr)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                flare["begin_time"],
                flare["max_time"],
                ...
                flare["max_xrlong"],
                flare["ts_unix"],
                flare.get("flare_gem_ordinal"),
                flare.get("flare_gem_dr"),
            ))
