...
Usage:
    ...
    meta_probs, take_trade = predict_meta(meta_result, base_pred_probs)
"""
...
def train_meta_model(oos_predictions, feature_data=None, feature_cols=None,
                     tf_name='1h', model_type='logistic', db_dir=None):
    ...
    assert not np.isnan(y_pred_probs).any(), f"NaN found in OOS predictions — {np.isnan(y_pred_probs).sum()} NaN values"
    ...
    if model_type == 'logistic':
        ...
        params = {
            ...
            'num_leaves': 4,
            'learning_rate': 0.1,
            'min_data_in_leaf': TF_MIN_DATA_IN_LEAF.get(tf_name, 3),
            'lambda_l2': 5.0,
            'lambda_l1': 2.0,
            'metric': 'auc',
            'verbosity': -1,
...
if __name__ == "__main__":
    ...
    if args.tf:
        # Production: load real OOS predictions and train meta model
        ...
        if not os.path.exists(oos_path):
            ...
            sys.exit(1)
        print(f"=== Meta-Labeling: {args.tf} ===")
        print(f"  Loading: {oos_path}")
        result = train_meta_model(oos_path, tf_name=args.tf, db_dir=args.db_dir)
        if result:
            print(f"  Model type: {result['model_type']}")
            print(f"  AUC: {result['metrics']['auc']:.3f}")
            print(f"  Threshold: {result['threshold']:.2f}")
            print(f"  Saved: meta_model_{args.tf}.pkl")
        else:
            print(f"  WARNING: Meta-labeling returned no model")
    else:
        # Test mode with synthetic data
        print("=== Meta-Labeling Test (synthetic) ===")
        np.random.seed(42)
        fake_oos = []
        for i in range(10):
            n = 300
            ...
            for j in range(n):
                y_probs[j, y_true[j]] += 0.2
            y_probs /= y_probs.sum(axis=1, keepdims=True)
            fake_oos.append({
                'path': i, 'y_true': y_true, 'y_pred_probs': y_probs,
