...
Phases:
  ...
  2. Training           — v2_multi_asset_trainer.py (LightGBM)
  3. Optimization       — exhaustive_optimizer.py (Optuna TPE) per tf
  4. Validation         — PBO + Deflated Sharpe from OOS predictions
  5. Meta-Labeling      — Logistic/shallow LightGBM meta-model per tf
...
REQUIRED = {'lightgbm': 'lightgbm', 'sklearn': 'scikit-learn', 'scipy': 'scipy',
            ...
            'numba': 'numba', 'torch': 'torch', 'lightgbm': 'lightgbm', 'optuna': 'optuna',
            'cupy': 'cupy-cuda12x',
            'pandas': 'pandas', 'dask': 'dask',
            'distributed': 'distributed', 'pyarrow': 'pyarrow'}

# Optional GPU packages — check + warn, never pip install.
GPU_CHECK = {'cudf': 'cudf', 'cuml': 'cuml', 'dask_cuda': 'dask-cuda'}

def install_deps():
    """Only pip-install packages that fail to import. GPU packages are checked but not installed."""
    ...
    # Check optional GPU packages (not pip-installable)
    for imp, pkg in GPU_CHECK.items():
        try:
            __import__(imp)
        except ImportError:
            log(f"INFO: {pkg} not available — optional GPU package, not pip-installable")
...
def run_phase_1(state, manifest, args):
    ...
    build_tfs = args.build_tf or ['4h', '1h', '15m']
...
def run_phase_3(state, manifest, args):
    ...
    for tf in args.tf:
        ...
        if is_step_done(manifest, 3, step_id):
            log(f"SKIP {step_id} (done)")
            continue
        config_file = os.path.join(V2_DIR, f'optuna_configs_{tf}.json')
    ...
    cmd = f"{sys.executable} -u exhaustive_optimizer.py {tf_str} --n-trials 200"
...
def get_phase_artifacts(phase):
    ...
    p = {
        ...
        2: ['model_v2_*.*', 'importance_v2_*.json',
            ...
            'features_v2_*.json'],
        3: ['optuna_configs_*.json', 'optimizer_checkpoint_*.json'],
...
def log_data_inventory():
    cats = [('Parquets', 'features_*.parquet'),
            ...
            ('Opt configs', 'optuna_configs_*.json'),
...
def print_dry_run_plan(args, manifest):
    ...
    for p in phases:
        ...
        if p == 1:
            ...
            for tf in (args.build_tf or ['4h', '1h', '15m']):
            ...
            for tf in args.tf:
                ex = os.path.exists(os.path.join(V2_DIR, f'optuna_configs_{tf}.json'))
...
""")
    ...
    parser.add_argument('--engine', default='lightgbm',
                        choices=['lightgbm'])
