...
15-minute to weekly candles with full volume data.
...
def download_candles(symbol="BTC/USDT", timeframe="1d", since_str="2017-08-17", limit_per_req=1000):
    ...
    conn = init_db()
    # Use Binance global for full history (public OHLCV API works globally, no auth needed)
    # Binance.US has limited history (launched 2019) — caused 4h/1h to only have 2024+ data
    try:
        exchange = ccxt.binance({"enableRateLimit": True})
        exchange.load_markets()
    except Exception as e:
        print(f"  WARNING: Binance global unavailable ({e}), trying Binance.US (limited history, may not have pre-2019 data)")
        try:
...
def download_all():
    ...
    timeframes = [
        ...
        ("4h", "2017-08-17"),
        ("1h", "2017-08-17"),
        ("15m", "2017-11-01"),   # 15m from Nov 2017 (Binance listing)
    ]
