...
for _pat in _stale_patterns:
    for _stale in glob.glob(_pat):
        # Only delete if it doesn't match the current TF
        # (features_{TF}_all.json will be regenerated by training)
        log(f"  Removing stale artifact: {_stale}")
        os.remove(_stale)
        _stale_count += 1
if _stale_count:
...
log(f"Cross gen phase: OMP_NUM_THREADS=4, NUMBA_NUM_THREADS={_numba_threads}")
