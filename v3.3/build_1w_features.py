...
=====================
Build weekly (1W) feature matrix from BTC/USDT weekly candles using the unified
...
Saves to features_1w.db, table features_1w.
"""
...
# ============================================================
print("=" * 70)
print("BUILD 1W FEATURE MATRIX (unified pipeline)")
print("=" * 70)

# --- 1W BTC candles (primary OHLCV) ---
print(f"\n{elapsed()} Loading 1W BTC/USDT candles...")
...
# ============================================================
print("=" * 70)
print("Computing 1W features via feature_library.build_all_features()...")
print("=" * 70)
...
# ============================================================
# SAVE TO features_1w.db
# ============================================================
print(f"\n{elapsed()} Saving to features_1w.db...")
...
parquet_path = f'{DB_DIR}/features_1w.parquet'
...
try:
    out_db = f'{DB_DIR}/features_1w.db'
    conn_out = sqlite3.connect(out_db)
    df_save.to_sql('features_1w', conn_out, if_exists='replace', index=False)
    conn_out.execute("CREATE INDEX IF NOT EXISTS idx_1w_ts ON features_1w(timestamp)")
    conn_out.close()
except Exception as e:
...
print(f"\n{'='*70}")
print(f"1W FEATURE MATRIX COMPLETE")
print(f"{'='*70}")
