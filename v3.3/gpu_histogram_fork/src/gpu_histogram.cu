...
/* ===================================================================
 * Environment variable flags for CUDA-Speed optimizations.
 * Each can be toggled at runtime via env vars.
 * =================================================================== */
static bool _env_checked = false;
static bool _use_warp_reduce   = false;  /* CUDA_WARP_REDUCE=1 */
static bool _use_batch_h2d     = false;  /* CUDA_BATCH_H2D=1 */

static void _check_env_flags() {
    if (_env_checked) return;
    _env_checked = true;
    const char* v;
    v = getenv("CUDA_WARP_REDUCE");
    _use_warp_reduce = (v && (v[0] == '1' || v[0] == 'y' || v[0] == 'Y'));
    v = getenv("CUDA_BATCH_H2D");
    _use_batch_h2d = (v && (v[0] == '1' || v[0] == 'y' || v[0] == 'Y'));
    if (_use_warp_reduce)
        fprintf(stderr, "[gpu_hist] CUDA_WARP_REDUCE=1: warp-cooperative atomics ENABLED\n");
    if (_use_batch_h2d)
        fprintf(stderr, "[gpu_hist] CUDA_BATCH_H2D=1: batched H2D transfers ENABLED\n");
}
...
struct GpuHistContext_ {
    ...
    /* ---- Batch H2D (Optimization A) ---- */
    int32_t*  d_batch_leaf_rows;   /* [n_rows] combined buffer for batched leaf rows */
    int32_t*  d_batch_offsets;     /* [max_batch+1] offsets into d_batch_leaf_rows */
    int32_t*  h_batch_offsets;     /* host-side offset accumulator */
    int32_t*  h_batch_leaf_buf;    /* pinned host staging for batched leaf rows */
    int32_t   batch_capacity;      /* max total rows across all batched leaves */
    int32_t   batch_count;         /* number of leaves in current batch */
    int32_t   batch_max_leaves;    /* max leaves per batch (63) */
...
/* ===================================================================
 * KERNEL 1b: Sparse Histogram Build — Warp-Cooperative Atomics
 *
 * Optimization B: Uses __shfl_down_sync warp-level reduction before
 * atomicAdd. Threads in the same warp that happen to write to the same
 * histogram bin are reduced first, so only lane 0 of each unique-bin
 * group does the atomic. For ultra-sparse data this reduces atomic
 * pressure by ~32x in the worst case.
 *
 * Enabled by CUDA_WARP_REDUCE=1 environment variable.
 *
 * Approach: Each thread processes one leaf row (same as kernel 1).
 * For each CSR nonzero, the thread participates in a warp-level
 * reduction: threads with the same bin index reduce their grad/hess
 * values, and only the last matching lane performs the atomicAdd.
 *
 * Because our data is ultra-sparse (millions of features, hundreds of
 * rows), bin collisions within a warp are rare. The main benefit is
 * on larger leaves where more threads are active.
 * =================================================================== */

__device__ __forceinline__ double warp_reduce_by_key_f64(
    int32_t my_key, double my_val, unsigned active_mask
) {
    /* Warp-cooperative reduction: accumulate values with matching keys.
     * Uses butterfly reduction pattern with __shfl_down_sync.
    ...
    for (int offset = 16; offset > 0; offset >>= 1) {
        ...
        double  other_val = __shfl_down_sync(active_mask, my_val, offset);
        if (other_key == my_key) {
            my_val += other_val;
        }
    }
...
__device__ __forceinline__ bool warp_is_key_leader(
    int32_t my_key, unsigned active_mask
) {
    /* Check if this lane is the lowest-numbered active lane with this key.
     * If so, it is the "leader" and should do the atomicAdd.
     *
     * IMPORTANT: active_mask must reflect actually active lanes. */
    int lane = threadIdx.x & 31;
    for (int offset = 1; offset <= 16; offset <<= 1) {
        int32_t other_key = __shfl_up_sync(active_mask, my_key, offset);
        /* Only compare if the source lane is active and exists */
        unsigned src_lane = lane - offset;
        if (lane >= offset && (active_mask & (1u << src_lane)) && other_key == my_key) {
            return false;  /* a lower active lane has the same key */
        }
    }
    return true;
}

__global__ void sparse_hist_build_warp_kernel(
    const int64_t* __restrict__ indptr,
    const int32_t* __restrict__ indices,
    ...
    /* Process CSR nonzeros with warp-cooperative reduction.
     * For each nonzero, active lanes in the warp process their own
     * nonzero entry simultaneously. We check if any lanes share the
    ...
    if (data == nullptr) {
        ...
        for (int64_t j = start; j < end; j++) {
            ...
            /* Only the leader lane (lowest with this key) does the atomic */
            if (warp_is_key_leader(col, active)) {
                atomicAdd(&hist[col * 2],     g_reduced);
                atomicAdd(&hist[col * 2 + 1], h_reduced);
            }
        }
    } else {
        /* EFB mode */
...
/* ===================================================================
 * KERNEL 8: Batched Histogram Build — Optimization A
 *
 * Processes multiple leaves in a single kernel launch. The leaf rows
 * are concatenated into d_batch_leaf_rows with offsets in d_batch_offsets.
 * Each thread determines which leaf it belongs to via the offset array.
 *
 * This eliminates per-leaf H2D transfers (63,000 -> ~1,000 per tree)
 * and amortizes kernel launch overhead across all leaves.
 * =================================================================== */

__global__ void sparse_hist_build_batched_kernel(
    const int64_t* __restrict__ indptr,
    const int32_t* __restrict__ indices,
    const uint8_t* __restrict__ data,
    const double*  __restrict__ gradients,
    const double*  __restrict__ hessians,
    const int32_t* __restrict__ batch_leaf_rows,  /* concatenated leaf rows */
    const int32_t* __restrict__ batch_offsets,     /* [n_leaves+1] */
    int32_t                     total_rows,        /* sum of all leaf row counts */
    int32_t                     class_id,
    int32_t                     num_classes,
    double*        __restrict__ hist               /* single histogram output */
) {
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= total_rows) return;

    int32_t row = batch_leaf_rows[tid];
    double g = gradients[row * num_classes + class_id];
    double h = hessians[row * num_classes + class_id];
...
GpuHistError gpu_hist_init(
    ...
    *handle = nullptr;
    ...
    /* ---- Batch H2D buffers (Optimization A) ---- */
    _check_env_flags();

    if (_use_batch_h2d) {
        ctx->batch_max_leaves = 63;
        ctx->batch_capacity = (int32_t)ctx->n_rows;  /* worst case: all rows */
        ctx->batch_count = 0;

        CUDA_CHECK_OOM(
            cudaMalloc(&ctx->d_batch_leaf_rows,
                       (size_t)ctx->batch_capacity * sizeof(int32_t)),
            fail_cleanup);
        ctx->gpu_bytes_allocated += (size_t)ctx->batch_capacity * sizeof(int32_t);

        CUDA_CHECK_OOM(
            cudaMalloc(&ctx->d_batch_offsets,
                       (size_t)(ctx->batch_max_leaves + 1) * sizeof(int32_t)),
            fail_cleanup);
        ctx->gpu_bytes_allocated += (size_t)(ctx->batch_max_leaves + 1) * sizeof(int32_t);

        /* Pinned host staging */
        CUDA_CHECK_OOM(
            cudaMallocHost(&ctx->h_batch_leaf_buf,
                           (size_t)ctx->batch_capacity * sizeof(int32_t)),
            fail_cleanup);
        ctx->h_batch_offsets = (int32_t*)calloc(ctx->batch_max_leaves + 1, sizeof(int32_t));
    }
...
fail_cleanup:
    ...
    if (ctx->d_batch_leaf_rows) cudaFree(ctx->d_batch_leaf_rows);
    if (ctx->d_batch_offsets)   cudaFree(ctx->d_batch_offsets);
...
GpuHistError gpu_hist_build(
    ...
    if (use_tiled) {
         ...
         * Uses __shfl_down_sync to reduce within warp before atomicAdd.
         * Best for larger leaves where warp-level bin collisions are possible. */
        sparse_hist_build_warp_kernel
...
GpuHistError gpu_hist_cleanup(GpuHistHandle handle) {
    ...
    /* Free batch H2D buffers (Optimization A) */
    if (ctx->d_batch_leaf_rows)  cudaFree(ctx->d_batch_leaf_rows);
    if (ctx->d_batch_offsets)    cudaFree(ctx->d_batch_offsets);
    if (ctx->h_batch_leaf_buf)   cudaFreeHost(ctx->h_batch_leaf_buf);
    if (ctx->h_batch_offsets)    free(ctx->h_batch_offsets);
