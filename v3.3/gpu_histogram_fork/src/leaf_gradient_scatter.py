...
# ---------------------------------------------------------------------------
# Optimization C: Vectorized batch kernel — 2 launches instead of 48
# All leaf rows concatenated into one buffer, launched as single kernel.
# Enabled by CUDA_VECTORIZE_LAUNCH=1 environment variable.
# ---------------------------------------------------------------------------

_VECTORIZE_LAUNCH = os.environ.get('CUDA_VECTORIZE_LAUNCH', '0') in ('1', 'y', 'Y', 'yes')


_WARP_REDUCE_SCATTER_F64 = r"""
extern "C" __global__
void warp_reduce_scatter_f64(
    const long long*  __restrict__ indptr,
    const int*        __restrict__ indices,
    const double*     __restrict__ gradients,
    const double*     __restrict__ hessians,
    const int*        __restrict__ all_leaf_rows,
    double*           __restrict__ hist_grad,
    double*           __restrict__ hist_hess,
    int total_leaf_rows
) {
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= total_leaf_rows) return;

    int row = all_leaf_rows[tid];
    double g = gradients[row];
    double h = hessians[row];
    ...
    for (long long j = start; j < end; j++) {
        ...
        for (int offset = 16; offset > 0; offset >>= 1) {
            int    other_col = __shfl_down_sync(active, col, offset);
            double other_g   = __shfl_down_sync(active, g_val, offset);
            double other_h   = __shfl_down_sync(active, h_val, offset);
            if (other_col == col) {
                g_val += other_g;
                h_val += other_h;
            }
        ...
        /* Check if this lane is the leader (no lower active lane has same col) */
        bool is_leader = true;
        for (int offset = 1; offset <= 16; offset <<= 1) {
            int other_col = __shfl_up_sync(active, col, offset);
            unsigned src_lane = lane - offset;
            if (lane >= offset && (active & (1u << src_lane)) && other_col == col) {
                is_leader = false;
                break;
            }
        }
        ...
        if (is_leader) {
            atomicAdd(&hist_grad[col], g_val);
            atomicAdd(&hist_hess[col], h_val);
        }
    }
}
"""
...
class LeafGradientScatter:
    ...
    def __init__(
        ...
        # Optimization D: CSR.T is always cached on GPU (dual storage)
        self._dual_csr_stored = True

        opts = []
        if _VECTORIZE_LAUNCH:
            opts.append("VECTORIZE_LAUNCH")
        if os.environ.get('CUDA_WARP_REDUCE', '0') in ('1', 'y', 'Y'):
            opts.append("WARP_REDUCE")
        if os.environ.get('CUDA_DUAL_CSR', '0') in ('1', 'y', 'Y', 'yes'):
            opts.append("DUAL_CSR")
        opts_str = f", opts=[{','.join(opts)}]" if opts else ""

        logger.info(
            "LeafGradientScatter initialized: %d rows x %d features, "
            "NNZ=%s, dtype=%s, H2D=%.1fms, crossover=%.0f%%%s",
            self.n_rows, self.n_features, f"{self.nnz:,}",
            dtype, h2d_ms, _CROSSOVER_FRACTION * 100, opts_str,
        )
