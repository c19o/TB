...
Manages 4 concurrent execution models (4H, 1H, 15m, 1D/1W context)
...
from config import (FEE_RATE as CONFIG_FEE_RATE, RISK_LIMITS,
                    MAX_PORTFOLIO_DD as CONFIG_MAX_PORTFOLIO_DD,
                    PROJECT_DIR)
...
# max_leverage capped to config.RISK_LIMITS['max_leverage'] (absolute hard cap)
_MAX_LEV = RISK_LIMITS['max_leverage']
DEFAULT_RISK = {
    '4h': {'max_risk_pct': 0.75, 'max_concurrent': 5, 'max_leverage': min(25, _MAX_LEV), 'priority_weight': 1.3},
    '1h': {'max_risk_pct': 0.25, 'max_concurrent': 3, 'max_leverage': min(50, _MAX_LEV), 'priority_weight': 1.1},
    '15m': {'max_risk_pct': 0.15, 'max_concurrent': 3, 'max_leverage': min(60, _MAX_LEV), 'priority_weight': 1.0},
}
...
class Position:
    ...
    def update(self, price, fee_rate=None):
        """Check exits. Returns PnL if closed, None if still open."""
        if fee_rate is None:
            fee_rate = CONFIG_FEE_RATE
        self.bars_held += 1
...
class PortfolioAggregator:
    def __init__(self, initial_balance=10000, max_total_heat=0.03,
                 max_portfolio_dd=None, fee_rate=None):
        self.initial_balance = initial_balance
        self.balance = initial_balance
        self.peak_balance = initial_balance
        self.max_total_heat = max_total_heat  # 3% max total exposure
        self.max_portfolio_dd = max_portfolio_dd if max_portfolio_dd is not None else CONFIG_MAX_PORTFOLIO_DD
        self.fee_rate = fee_rate if fee_rate is not None else CONFIG_FEE_RATE
...
class MultiTFPredictor:
    ...
    def __init__(self):
        ...
        for tf in ['4h', '1h', '15m']:
            ...
            # Try features_{tf}_all.json first (full feature set), fall back to pruned
