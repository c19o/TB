...
import sys, os, io, time, json, warnings, argparse
if os.name == 'nt':
...
try:
    from config import (TF_PARENT_MAP, TRADE_TYPE_PARAMS, TRADE_THRESHOLDS as CFG_TRADE_THRESHOLDS,
                        FEE_RATE as CONFIG_FEE_RATE, STARTING_BALANCE as CONFIG_STARTING_BALANCE,
                        REGIME_MULT as CONFIG_REGIME_MULT, REGIME_SLOPE_THRESHOLD,
                        REGIME_CRASH_VOL_MULT, REGIME_CRASH_DD_THRESHOLD,
                        KELLY_SAFETY_FRACTION, KELLY_MAX_RISK_MULT,
...
# Regime multipliers from config.py (single source of truth)
if _HAS_CONFIG:
    REGIME_MULT = CONFIG_REGIME_MULT
...
def load_full_history(tf_name):
    ...
    if os.path.exists(oos_path):
        ...
        # IS metrics gate — detect potential overfitting before using predictions
        for fold in oos_preds:
            is_accuracy = fold.get('is_accuracy')
            is_sharpe = fold.get('is_sharpe')
            if is_accuracy is not None and is_accuracy > 0.95:
                raise ValueError(
                    f"IS accuracy {is_accuracy:.4f} > 0.95 on CPCV path {fold.get('path_idx', '?')} "
                    f"— likely overfitting, audit failed. Retrain with stronger regularization."
                )
            if is_sharpe is not None and is_sharpe > 50.0:
                raise ValueError(
                    f"IS Sharpe {is_sharpe:.2f} > 50.0 on CPCV path {fold.get('path_idx', '?')} "
                    f"— likely overfitting, audit failed. Retrain with stronger regularization."
                )
        ...
        for level in ['god_mode', 'aggressive', 'balanced']:
            ...
            if c and 'params' in c:
                ...
                return {
                    ...
                    'conf_thresh': p[6] if len(p) > 6 else (DEFAULT_CONF_THRESH if _HAS_CONFIG else 0.6),
                }

    # Sensible defaults
    _def_ct = DEFAULT_CONF_THRESH if _HAS_CONFIG else 0.6
    print(f"  WARNING: No config found for {tf_name}, using defaults", flush=True)
    return {
        'leverage': 5, 'risk_pct': 1.0, 'stop_atr': 0.5,
        'rr': 2.0, 'max_hold': 24, 'exit_type': 0, 'conf_thresh': _def_ct,
    }
...
def detect_regime_series(closes):
    ...
    above = closes > sma100
    _slope_thresh = REGIME_SLOPE_THRESHOLD if _HAS_CONFIG else 0.001
    _crash_vol = REGIME_CRASH_VOL_MULT if _HAS_CONFIG else 2.0
    _crash_dd = REGIME_CRASH_DD_THRESHOLD if _HAS_CONFIG else 0.15
    bull_mask = above & (slope > _slope_thresh)
    bear_mask = (~above) & (slope < -_slope_thresh)

    # Crash = volatility > Nx 90-day avg AND below SMA100 AND drawdown from 30-day high > threshold
    crash_mask = (rvol_20 > _crash_vol * rvol_90_avg) & (~above) & (dd_from_30h > _crash_dd)
...
def _simulate_with_trades(timestamps_i64, dirs, confs, closes, highs, lows, atrs, regimes,
                          ...
                          kelly_safety=0.25, kelly_max_mult=3.0,
                          dd_halt=0.15, dd_steepness=2.0):
    """
    ...
    for t in range(n):
        ...
        if in_trade:
            ...
            if hit_sl or hit_tp or time_exit:
                ...
                # Drawdown scaling: reduce size in drawdown (params from config)
                dd = (peak_balance - balance) / max(peak_balance, 1e-8)
                if dd < dd_halt:
                    dd_scale = max(0.0, 1.0 - dd_steepness * dd)
                else:
                    dd_scale = 0.0
        ...
        if not in_trade and balance > 0.0:
            if dirs[t] != 0 and confs[t] > conf_thresh:
                ...
                # Kelly sizing: boost risk based on confidence (params from config)
                kelly_f = (confs[t] - conf_thresh) / max(1.0 - conf_thresh, 1e-8)
                kelly_risk = eff_risk * (1.0 + kelly_safety * kelly_f)
                kelly_risk = min(kelly_risk, eff_risk * kelly_max_mult)
...
def main():
    ...
    parser.add_argument('--tf', nargs='+', default=['15m', '1h', '4h', '1d', '1w'],
    ...
    print(f"  LGB mode:  CPU (force_col_wise)")
    ...
    for tf in args.tf:
        ...
        # Kelly/DD params from config (passed explicitly to numba)
        _kelly_s = float(KELLY_SAFETY_FRACTION) if _HAS_CONFIG else 0.25
        _kelly_m = float(KELLY_MAX_RISK_MULT) if _HAS_CONFIG else 3.0
        _dd_h = float(DD_HALT_THRESHOLD) if _HAS_CONFIG else 0.15
        _dd_st = float(DD_SCALE_STEEPNESS) if _HAS_CONFIG else 2.0
        ...
        equity, trades_arr, n_trades = _simulate_with_trades(
            ...
            _kelly_s, _kelly_m, _dd_h, _dd_st,
        )
...
from config import TF_SLIPPAGE, load_tf_allocation
...
def run_unified_backtest(tf_list=None):
    ...
    chronologically aligned to 15-minute bars. At each bar close, checks
    ...
    base_tf = '15m' if '15m' in tf_data else loaded_tfs[0]
    ...
    for t_idx in range(n_timeline):
        ...
        for pos in active_positions:
            ...
            if hit_sl or hit_tp or time_exit:
                ...
                # DD scaling (config values)
                _dd_h2 = DD_HALT_THRESHOLD if _HAS_CONFIG else 0.15
                _dd_s2 = DD_SCALE_STEEPNESS if _HAS_CONFIG else 2.0
                dd = (peak_balance - balance) / max(peak_balance, 1e-8)
                dd_scale = max(0.0, 1.0 - _dd_s2 * dd) if dd < _dd_h2 else 0.0
        ...
        _dd_halt_mt = DD_HALT_THRESHOLD if _HAS_CONFIG else 0.15
        if portfolio_dd > _dd_halt_mt:
        ...
        if balance > 0 and not portfolio_halted:
            ...
            for ti, tf in enumerate(loaded_tfs):
                ...
                # Kelly sizing (config values)
                _ks = KELLY_SAFETY_FRACTION if _HAS_CONFIG else 0.25
                _km = KELLY_MAX_RISK_MULT if _HAS_CONFIG else 3.0
                kelly_f = (confidence - conf_thresh) / max(1.0 - conf_thresh, 1e-8)
                kelly_risk = eff_risk * (1.0 + _ks * kelly_f)
                kelly_risk = min(kelly_risk, eff_risk * _km)
                ...
                # DD scaling on entry (config value)
                _ddh3 = DD_HALT_THRESHOLD if _HAS_CONFIG else 0.15
                dd = (peak_balance - balance) / max(peak_balance, 1e-8)
                if dd >= _ddh3:
                    continue
