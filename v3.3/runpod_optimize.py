...
def main():
    ...
    try:
        ...
        ssh_cmd(host, port,
            "pip install lightgbm numpy pandas scikit-learn scipy hmmlearn 2>&1 | tail -3 && "
        ...
        # Run optimizer
        print("\nRunning Optuna optimizer on 4x RTX 5090...")
        print("This will take a while (2.88B combos)...\n")

        # Run single optimizer process — it internally uses ProcessPoolExecutor with
        # cp.cuda.Device(gpu_id).use() to route TFs across all visible GPUs.
        # No CUDA_VISIBLE_DEVICES pinning (all GPUs visible, internal routing handles it).
        opt_cmd = (
            "cd %s && "
            "SAVAGE22_DB_DIR=%s python -u exhaustive_optimizer.py "
            "--tf 5m --tf 15m --tf 1h --tf 4h --tf 1d --tf 1w "
            "> optimizer.log 2>&1 && echo ALL_DONE"
        ...
        for f in ["optuna_configs.json", "optimizer.log"]:
            remote = REMOTE_DIR + "/" + f
