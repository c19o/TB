...
def check_config_params():
    ...
    check("feature_fraction_bynode >= 0.5",
          p.get('feature_fraction_bynode', 0) >= 0.5,
          f"feature_fraction_bynode={p.get('feature_fraction_bynode')} -- must be >= 0.5. "
...
def check_optuna_search_space():
    ...
    if 'feature_fraction_bynode' in bounds:
        ...
        check("Optuna feature_fraction_bynode lower >= 0.5",
              low is not None and low >= 0.5,
              f"feature_fraction_bynode search lower={low} -- must be >= 0.5. "
              f"FIX: run_optuna_local.py")

    if 'bagging_fraction' in bounds:
        low, _ = bounds['bagging_fraction']
        check("Optuna bagging_fraction lower >= 0.5",
              low is not None and low >= 0.5,
              f"bagging_fraction search lower={low} -- must be >= 0.5")
    ...
    if 'min_gain_to_split' in bounds:
        low, _ = bounds['min_gain_to_split']
        check("Optuna min_gain_to_split lower >= 0.5",
              low is not None and low >= 0.5,
              f"min_gain_to_split search lower={low} -- must be >= 0.5")
