...
try:
    if _CUDA_MAJOR >= 13:
        raise ImportError("CUDA 13+ — cuDF disabled")
    import cudf
    _HAS_CUDF = _HAS_GPU
    if _HAS_CUDF:
...
try:
    if _CUDA_MAJOR >= 13:
        raise ImportError("CUDA 13+ driver — cuML disabled (binary compat issue)")
...
def compute_esoteric_features(df: pd.DataFrame, tweets_df: pd.DataFrame,
    ...
    if news_df is not None and len(news_df) > 0:
        ...
        try:
            ...
            import logging; logging.getLogger('feature_library').warning(f"News feature computation failed: {_e}")
    ...
    try:
        ...
        import logging; logging.getLogger('feature_library').warning(f"Gematria feature computation failed: {_e}")
...
def compute_event_astrology(df, tweets_df, news_df, sports_df,
    ...
    if tweets_df is not None and len(tweets_df) > 0 and 'ts_unix' in tweets_df.columns:
        try:
            ...
            import logging; logging.getLogger('feature_library').warning(f"Tweet astrology failed: {_e}")
    ...
    if news_df is not None and len(news_df) > 0 and 'ts_unix' in news_df.columns:
        try:
            ...
            import logging; logging.getLogger('feature_library').warning(f"News astrology failed: {_e}")
    ...
    if sports_df is not None:
        ...
        if games_df is not None and len(games_df) > 0:
            try:
                ...
                import logging; logging.getLogger('feature_library').warning(f"Game astrology failed: {_e}")
...
def compute_knn_features(df: pd.DataFrame, tf_name: str = '1h') -> pd.DataFrame:
    ...
    try:
        ...
        import logging; logging.getLogger('feature_library').warning(f"KNN features failed: {_e}")
    return out.to_pandas() if hasattr(out, 'to_pandas') else out
...
def compute_space_weather_features(df: pd.DataFrame,
    ...
    space_weather.db and/or kp_history_gfz.txt and forward-fill to match bar
...
def build_all_features(ohlcv: pd.DataFrame, esoteric_frames: dict,
    ...
    Args:
        ...
        space_weather_df: DataFrame (DatetimeIndex) from space_weather.db
            and/or kp_history_gfz.txt, forward-filled to bar frequency.
...
def _add_cross_features(df: pd.DataFrame):
    ...
    try:
        ...
        import logging; logging.getLogger('feature_library').warning(f"Systematic cross expansion failed: {_e}")
