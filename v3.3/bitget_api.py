...
def _request(method, path, params=None, body=None, authenticated=False):
    """Make HTTP request to Bitget API with retry logic (8B.8)."""
    max_retries = 3
        ...
        try:
            with urllib.request.urlopen(req, timeout=15) as resp:
                ...
                if data.get("code") == "00000":
                    ...
                    # API-level rejection — don't retry (intentional)
            ...
            if e.code >= 500 and attempt < max_retries - 1:
                ...
                wait = 2 ** attempt
                print(f"[BITGET HTTP {e.code}] retrying in {wait}s (attempt {attempt+1}/{max_retries})")
                time.sleep(wait)
                continue
            ...
            if attempt < max_retries - 1:
                wait = 2 ** attempt
                print(f"[BITGET NETWORK ERROR] {e} — retrying in {wait}s (attempt {attempt+1}/{max_retries})")
                time.sleep(wait)
                continue
            print(f"[BITGET ERROR] {e} (all {max_retries} attempts failed)")
            return None
...
def get_ticker(symbol="BTCUSDT"):
    """Get current price for a symbol. 8B.7: Bitget primary, Binance.US fallback."""
    # Primary: Bitget (our trading venue — most accurate for execution)
    ...
    # Fallback: Binance.US (public, no auth needed)
    try:
...
def get_klines(symbol="BTCUSDT", interval="4h", limit=100):
    """Get candle data. 8B.7: Bitget primary, Binance.US fallback."""
    # Primary: Bitget candles
    bitget_interval_map = {"1m": "1m", "5m": "5m", "15m": "15m", "1h": "1H",
                           "4h": "4H", "1d": "1D", "1w": "1W"}
    bg_interval = bitget_interval_map.get(interval, interval)
    try:
        data = _request("GET", "/api/v2/mix/market/candles",
                        params={"symbol": SYMBOL, "productType": PRODUCT_TYPE,
        ...
        print(f"[BITGET KLINES ERROR] {e}")

    # Fallback: Binance.US (public, no auth)
    ...
    try:
        ...
        print(f"[KLINES FALLBACK ERROR] {e}")
        return []
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if cmd == "balance":
            ...
            for k in klines[-5:]:
                print(f"  {datetime.fromtimestamp(k['time']/1000, tz=timezone.utc).strftime('%Y-%m-%d %H:%M')} "
