...
CREATE TABLE IF NOT EXISTS streamer_articles (
    ...
    word_count              INTEGER,
    pub_dr_date             INTEGER,
    pub_day_of_year         INTEGER,
    pub_week_number         INTEGER,
    pub_is_caution_date     INTEGER,
    pub_is_pump_date        INTEGER,
    pub_is_friday_13        INTEGER,
    pub_is_btc_213_date     INTEGER,
    pub_is_master_date      INTEGER,
    inserted_at             TEXT DEFAULT (datetime('now'))
)
"""

_DATE_NUM_COLS = [
    "pub_dr_date", "pub_day_of_year", "pub_week_number",
    "pub_is_caution_date", "pub_is_pump_date", "pub_is_friday_13",
    "pub_is_btc_213_date", "pub_is_master_date",
]
...
def init_db():
    ...
    _ensure_date_numerology_cols(conn)
    conn.commit()
    return conn


def _ensure_date_numerology_cols(conn):
    """Add date numerology columns to existing DB if not present."""
    existing = {row[1] for row in conn.execute("PRAGMA table_info(streamer_articles)").fetchall()}
    for col, typedef in _DATE_NUM_DEFS.items():
        if col not in existing:
            conn.execute(f"ALTER TABLE streamer_articles ADD COLUMN {col} {typedef}")
...
def enrich(title: str, published_at: str | None, ts_unix: int | None) -> dict:
    ...
    # Date DR + numerology from the article's publish time (fall back to now)
    if ts_unix:
    ...
    return {
        ...
        # Date numerology signals
        "pub_dr_date": dn["dr_date"],
        "pub_day_of_year": dn["day_of_year"],
        "pub_week_number": dn["week_number"],
        "pub_is_caution_date": int(dn["is_caution_date"]),
        "pub_is_pump_date": int(dn["is_pump_date"]),
        "pub_is_friday_13": int(dn["is_friday_13"]),
        "pub_is_btc_213_date": int(dn["is_btc_213_date"]),
        "pub_is_master_date": int(dn["is_master_date"]),
    }
...
def insert_article(conn, source, title, url, published_at, ts_unix, enriched):
    ...
    try:
        ...
        """, (
            ...
            enriched["word_count"],
            enriched.get("pub_dr_date"),
            enriched.get("pub_day_of_year"),
            enriched.get("pub_week_number"),
            enriched.get("pub_is_caution_date"),
            enriched.get("pub_is_pump_date"),
            enriched.get("pub_is_friday_13"),
            enriched.get("pub_is_btc_213_date"),
            enriched.get("pub_is_master_date"),
        ))
        return True
