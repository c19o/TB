...
# Full names for gematria computation
TICKER_FULL_NAMES = {
    "spx":     "S&P Five Hundred",
    "nasdaq":  "NASDAQ",
    "dxy":     "Dollar Index",
    "gold":    "Gold",
    "vix":     "Volatility Index",
    "us10y":   "Ten Year Treasury",
    "russell":  "Russell Two Thousand",
    "oil":     "Crude Oil",
    "silver":  "Silver",
    "mstr":    "MicroStrategy",
    "coin":    "Coinbase",
    "hyg":     "High Yield Bond",
    "tlt":     "Treasury Long Bond",
    "ibit":    "iShares Bitcoin ETF",
}

# Precomputed static gematria for ticker full names (never changes)
TICKER_GEM = {name: _gematria(full) for name, full in TICKER_FULL_NAMES.items()}
...
def init_db():
    ...
    # Build column defs dynamically from TICKERS (price + DR per ticker)
    ticker_cols = ", ".join(f"{name} REAL" for name in TICKERS)
    dr_cols = ", ".join(f"{name}_dr INTEGER" for name in TICKERS)
    conn.execute(f"""
    ...
    """)
    # Also create a static ticker_gematria reference table
    conn.execute("""
        CREATE TABLE IF NOT EXISTS ticker_gematria (
            ticker TEXT PRIMARY KEY,
            full_name TEXT,
            gem_ordinal INTEGER,
            gem_reverse INTEGER,
            gem_reduction INTEGER,
            gem_dr INTEGER
        )
    """)
    _ensure_macro_cols(conn)
    _upsert_ticker_gematria(conn)
    conn.commit()
    return conn


def _ensure_macro_cols(conn):
    """Add DR columns to existing macro_data table if missing."""
    existing = {r[1] for r in conn.execute("PRAGMA table_info(macro_data)").fetchall()}
    for name in TICKERS:
        col = f"{name}_dr"
        if col not in existing:
            conn.execute(f"ALTER TABLE macro_data ADD COLUMN {col} INTEGER")


def _upsert_ticker_gematria(conn):
    """Upsert static ticker name gematria into reference table."""
    for name, g in TICKER_GEM.items():
        full = TICKER_FULL_NAMES[name]
        conn.execute("""
            INSERT OR REPLACE INTO ticker_gematria
            (ticker, full_name, gem_ordinal, gem_reverse, gem_reduction, gem_dr)
            VALUES (?, ?, ?, ?, ?, ?)
        """, (name, full, g["ordinal"], g["reverse"], g["reduction"], g["dr_ordinal"]))
...
def price_dr(value):
    ...
    For values >= 1.0: use integer part.
    For sub-1.0 values (e.g. VIX=0.85): multiply by 100 first so DR(0.85)=DR(85)=4.
    """
    ...
    try:
        ...
        int_val = int(abs_val)
        if int_val == 0:
            # sub-1.0 value: multiply by 100 to get meaningful digits
            # e.g. 0.85 → 85, DR(85) = 4
            int_val = int(abs_val * 100)
            if int_val == 0:
                return 0
...
def poll_once(conn):
    ...
    # Compute DR for each ticker's price
    drs = {name: price_dr(prices.get(name)) for name in TICKERS}

    # Build dynamic INSERT — price cols + DR cols
    price_col_names = ", ".join(TICKERS.keys())
    dr_col_names    = ", ".join(f"{name}_dr" for name in TICKERS)
    all_col_names   = f"{price_col_names}, {dr_col_names}"
    placeholders    = ", ".join(["?"] * (1 + len(TICKERS) * 2))
    values = ([date_str]
              + [prices.get(name, None) for name in TICKERS]
              + [drs.get(name) for name in TICKERS])
    ...
    if existing:
        # Update existing row with fresh prices + DRs
        set_clause = ", ".join(
            [f"{name} = ?" for name in TICKERS]
            + [f"{name}_dr = ?" for name in TICKERS]
        )
        update_vals = ([prices.get(name, None) for name in TICKERS]
                       + [drs.get(name) for name in TICKERS]
                       + [date_str])
        conn.execute(
        ...
        conn.execute(
            f"INSERT INTO macro_data (date, {all_col_names}) VALUES ({placeholders})",
            values
        )
