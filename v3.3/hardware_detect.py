...
def get_cpu_count() -> int:
    ...
    Containers may have fewer CPUs than the host. This returns the ACTUAL
    available CPU count, not the host's os.cpu_count().
    """
    ...
    # sched_getaffinity (respects taskset / cpuset)
    try:
...
def get_available_ram_gb() -> float:
    """Cgroup-aware AVAILABLE RAM in GB (not total, not host).
    In containers, psutil/proc reports HOST RAM. This returns the cgroup limit.
    """
    ...
    try:
        with open('/sys/fs/cgroup/memory.max') as f:
            val = f.read().strip()
            if val != 'max':
                cgroup_bytes = int(val)
                if cgroup_bytes < 100 * 1024**4:  # < 100TB = real limit
                    # Subtract current usage for "available"
                    try:
                        with open('/sys/fs/cgroup/memory.current') as fc:
                            used = int(fc.read().strip())
                        return max(0.0, (cgroup_bytes - used) / (1024**3))
                    except (FileNotFoundError, ValueError):
                        return cgroup_bytes / (1024**3)
    except (FileNotFoundError, ValueError, PermissionError):
        pass
    ...
    try:
        with open('/sys/fs/cgroup/memory/memory.limit_in_bytes') as f:
            ...
            if cgroup_bytes < 100 * 1024**4:  # < 100TB = real limit
                try:
                    with open('/sys/fs/cgroup/memory/memory.usage_in_bytes') as fu:
                        used = int(fu.read().strip())
                    return max(0.0, (cgroup_bytes - used) / (1024**3))
                except (FileNotFoundError, ValueError):
                    return cgroup_bytes / (1024**3)
    except (FileNotFoundError, ValueError, PermissionError):
