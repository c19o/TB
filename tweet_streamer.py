...
from scrape_twitter import (
    ...
    detect_tweet_colors, ensure_color_columns,
)
...
def ensure_sentiment_columns(conn):
    """Add sentiment and color columns to tweets table if they don't exist."""
    existing = set()
    ...
    try:
        ...
        for instruction in instructions:
            ...
            for entry in entries:
                ...
                # Color detection (already computed in parse_tweet_data, use those values)
                clr = {
                    'has_gold': tweet_data["has_gold"],
                    'has_red': tweet_data["has_red"],
                    'has_green': tweet_data["has_green"],
                    'dominant_colors': tweet_data["dominant_colors"],
                }
                ...
                try:
                    ...
                    """, (
                        ...
                        clr["has_gold"], clr["has_red"],
                        clr["has_green"], clr["dominant_colors"],
                    ))
                    new_count += 1
                    log.info(f"  NEW @{handle}: gem={gs['gematria_simple']} sent={gs['sentiment_score']} clr={clr['dominant_colors']} | {tweet_data['full_text'][:70]}...")
                except Exception:
                    pass
...
def backfill_gematria_sentiment(conn):
    """Fill gematria, sentiment, and colors for all existing tweets that are missing them."""
    ...
    log.info(f"  {len(rows)} tweets need gematria/sentiment backfill")
    ...
    conn.commit()
    log.info(f"  Gematria/sentiment backfill complete: {updated} tweets updated")

    # ---- Backfill colors for rows that have no color data ----
    log.info("  Backfilling color detection on existing tweets...")

    color_rows = conn.execute("""
        SELECT tweet_id, full_text, media_urls FROM tweets
        WHERE (has_gold = 0 AND has_red = 0 AND has_green = 0
               AND (dominant_colors IS NULL OR dominant_colors = ''))
        AND full_text IS NOT NULL AND full_text != ''
    """).fetchall()

    log.info(f"  {len(color_rows)} tweets need color backfill")

    color_updated = 0
    for tweet_id, text, media_urls in color_rows:
        clr = detect_tweet_colors(text, media_urls)
        # Only update if we actually detected a color
        if clr['has_gold'] or clr['has_red'] or clr['has_green']:
            conn.execute("""
                UPDATE tweets SET
                    has_gold = ?, has_red = ?, has_green = ?, dominant_colors = ?
                WHERE tweet_id = ?
            """, (
                ...
                clr['dominant_colors'], tweet_id,
            ))
            color_updated += 1
            if color_updated % 1000 == 0:
                conn.commit()
                log.info(f"  Color backfilled {color_updated}...")

    conn.commit()
    log.info(f"  Color backfill complete: {color_updated} tweets updated")

    return updated + color_updated
