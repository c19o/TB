...
if __name__ == "__main__":
    ...
    print("=== PBO Test (fallback half-split) ===")
    # Simulate 15 CPCV paths with random predictions
    np.random.seed(42)
    fake_oos = []
    for i in range(15):
        n = 200
        y_true = np.random.randint(0, 3, n)
        # Slightly better than random (60% accuracy)
    ...
    print(f"  PBO = {pbo['pbo']:.3f} ({pbo['recommendation']}) [method={pbo.get('method', '?')}]")
    print(f"  omega_bar = {pbo.get('omega_bar', 'N/A')}")
    print(f"  Best IS path OOS rank: {pbo['best_is_oos_rank']} / {pbo['n_paths']}")

    print("\n=== PBO Test (proper IS metrics) ===")
    # Simulate IS metrics from training
    fake_is_metrics = [{'path': i, 'is_accuracy': 0.55 + 0.02 * i, 'is_sharpe': 0.3 + 0.1 * i}
                       for i in range(15)]
    pbo_proper = compute_pbo(fake_oos, is_metrics=fake_is_metrics)
    print(f"  PBO = {pbo_proper['pbo']:.3f} ({pbo_proper['recommendation']}) [method={pbo_proper.get('method', '?')}]")
    print(f"  omega_bar = {pbo_proper.get('omega_bar', 'N/A'):.3f}")
    print(f"  Best IS path OOS rank: {pbo_proper['best_is_oos_rank']} / {pbo_proper['n_paths']}")

    print("\n=== Deflated Sharpe Test ===")
    # Test with realistic Sharpe
    ...
    print(f"  Expected max Sharpe (noise): {dsr['expected_max_sharpe']:.2f}")
    print(f"  DSR: {dsr['dsr']:.3f}")
    print(f"  p-value: {dsr['p_value']:.4f}")
    print(f"  Recommendation: {dsr['recommendation']}")

    print("\n=== Full Report Test (with returns + IS metrics) ===")
    report = validation_report(
        fake_oos,
        returns=np.random.randn(5000) * 0.01 + 0.0005,
        n_optimizer_trials=30_000_000,
        tf_name='1h',
        is_metrics=fake_is_metrics,
    )
    print(f"  Overall: {report['overall']}")
    print(f"  PBO: {report['pbo']['pbo']:.3f} [method={report['pbo'].get('method', '?')}]")
