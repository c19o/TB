...
try:
    # 1. Load kp_history.txt (GFZ Potsdam format)
    kp_rows = []
    kp_path = f'{DB_DIR}/kp_history.txt'
        ...
        with open(kp_path, 'r', encoding='utf-8', errors='replace') as fh:
            for line in fh:
                ...
                try:
                    ...
                    continue
        print(f"  kp_history.txt: {len(kp_rows)} daily rows")
    else:
        print(f"  kp_history.txt not found")
    ...
    # 3. Combine kp_history + space_weather.db
    if len(kp_df) > 0:
