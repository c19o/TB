...
# -*- coding: utf-8 -*-
"""
cloud_runner.py — Full 6-TF Pipeline with GPU Pinning
=======================================================
...
# Phase 2: 1h→15m→5m sequential (each gets full 320GB VRAM)

PHASE1_PARALLEL = ['1w', '1d', '4h']
PHASE2_SEQUENTIAL = ['1h', '15m', '5m']
...
def _get_optimizer_trial_count(tf):
    ...
    known = {'5m': 5_280_000, '15m': 5_808_000, '1h': 7_260_000,
...
def main():
    ...
    parser.add_argument('--tf', action='append', help='Specific TFs (default: all 6)')
    ...
    log(f"{'='*60}")
    log("FULL 6-TF PIPELINE — GPU-PINNED ASSEMBLY LINE")
    log(f"{'='*60}")
    ...
    if args.tf:
        ...
        # PHASE 2: Build + Train 1h → 15m → 5m sequential
        log(f"\n{'='*60}")
        log("PHASE 2: BUILD+TRAIN 1h, 15m, 5m SEQUENTIAL")
        log(f"{'='*60}")
