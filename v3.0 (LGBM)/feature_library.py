...
def _safe_reindex(series_or_df, target_index, **kwargs):
    """Reindex with automatic timezone stripping to prevent cudf.pandas fallback errors."""
    obj = series_or_df.copy()
    if hasattr(obj.index, 'tz') and obj.index.tz is not None:
        obj.index = obj.index.tz_localize(None)
    target = _strip_tz(target_index) if hasattr(target_index, 'tz') else target_index
    result = obj.reindex(target, **kwargs)
    result.index = target_index  # restore original index
    return result
...
def _df_mod(like):
    """Return cudf or pd module matching object type."""
    return cudf if (_HAS_CUDF and _is_gpu(like)) else pd
...
TF_CONFIG = {
    '5m': {
        'bucket_seconds': 300,
        'return_bars': [1, 12, 48, 288],     # 5m, 1h, 4h, 1d
        'lag_bars': [1, 12, 48, 96, 288],
        'vol_short': 12,
        'vol_long': 288,
        'vwap_window': 288,                   # 1 day
        'fg_lag_bars': [288, 864, 1440, 2880],
        'macro_roc_short': 288 * 5,
        'macro_roc_long': 288 * 20,
        'corr_window': 288 * 20,
        'hash_roc_bars': 288 * 7,
        'fg_roc_bars': 288 * 5,
        'ema50_slope_bars': 240,
    },
    '15m': {
        'bucket_seconds': 900,
...
def _reduce_date(dt):
    """Reduce a datetime to single-digit date numerology."""
    s = dt.strftime('%Y%m%d')
...
def _price_contains(price, pattern):
    """Check if integer price contains a pattern string."""
    if np.isnan(price):
        return 0
    return 1 if pattern in str(int(price)) else 0
...
def bucket_events(events_df, bar_index, bucket_seconds):
    """
    Assign events to candle bars by timestamp bucket.

    Args:
        events_df: DataFrame with 'ts_unix' column (numeric epoch seconds)
        bar_index: DatetimeIndex of candle bars
        bucket_seconds: Size of each bucket in seconds

    Returns:
        Series of bucket keys aligned to events_df rows,
        and a mapping from bucket keys to bar_index positions.
    """
    ts = pd.to_numeric(events_df['ts_unix'], errors='coerce')
    event_buckets = (ts // bucket_seconds) * bucket_seconds
...
def compute_ta_features(df: pd.DataFrame, tf_name: str = '1h') -> pd.DataFrame:
    ...
    for i in range(2, n):
        ...
        if h_arr[i - 2] < l_arr[i]:
            fvg_bull[i] = 1
        # Bearish FVG: candle[i-2].low > candle[i].high (gap down unfilled)
        if l_arr[i - 2] > h_arr[i]:
            fvg_bear[i] = 1
    out['fvg_bullish'] = fvg_bull
    out['fvg_bearish'] = fvg_bear

    # FVG nearest distance — track unfilled FVGs and find closest
    fvg_nearest_dist = np.full(n, np.nan)
    active_bull_fvgs = []  # list of (gap_low, gap_high) = (h[i-2], l[i])
    active_bear_fvgs = []  # list of (gap_low, gap_high) = (h[i], l[i-2])
    for i in range(2, n):
        # Register new FVGs
        if fvg_bull[i]:
            active_bull_fvgs.append((h_arr[i - 2], l_arr[i]))
        if fvg_bear[i]:
            active_bear_fvgs.append((h_arr[i], l_arr[i - 2]))

        # Remove filled FVGs: bull FVG filled when price trades through it (low <= gap_high)
        active_bull_fvgs = [(gl, gh) for gl, gh in active_bull_fvgs if l_arr[i] > gl]
        active_bear_fvgs = [(gl, gh) for gl, gh in active_bear_fvgs if h_arr[i] < gh]

        # Cap active lists to prevent unbounded growth
        if len(active_bull_fvgs) > 50:
            active_bull_fvgs = active_bull_fvgs[-50:]
        if len(active_bear_fvgs) > 50:
            active_bear_fvgs = active_bear_fvgs[-50:]

        # Find nearest FVG distance
        min_dist = np.inf
        for gl, gh in active_bull_fvgs:
            mid_fvg = (gl + gh) / 2.0
            dist = abs(c_arr[i] - mid_fvg)
            if dist < min_dist:
                min_dist = dist
        for gl, gh in active_bear_fvgs:
            mid_fvg = (gl + gh) / 2.0
            dist = abs(c_arr[i] - mid_fvg)
            if dist < min_dist:
                min_dist = dist
    ...
    # Break of Structure (BOS) — using 20-bar swing lookback
    bos_n = 20
    bos_dir = np.zeros(n, dtype=np.float64)
    for i in range(bos_n, n):
        highest_high = np.max(h_arr[i - bos_n:i])
        lowest_low = np.min(l_arr[i - bos_n:i])
    ...
    # Liquidity Sweep — price exceeded prior swing, then reversed
    liq_sweep = np.zeros(n, dtype=np.float64)
    for i in range(bos_n + 1, n):
        swing_high = np.max(h_arr[i - bos_n:i])
        swing_low = np.min(l_arr[i - bos_n:i])
        # Bull sweep: wick below swing low, close back inside
        if l_arr[i] < swing_low and c_arr[i] > swing_low:
            liq_sweep[i] = 1.0
        # Bear sweep: wick above swing high, close back inside
        elif h_arr[i] > swing_high and c_arr[i] < swing_high:
            liq_sweep[i] = -1.0
    out['liquidity_sweep'] = liq_sweep

    # Order Block distance — last opposite-direction candle before a BOS
    ob_dist = np.full(n, np.nan)
    last_bull_ob = np.nan  # price level of last bullish order block
    last_bear_ob = np.nan
    for i in range(bos_n + 1, n):
        if bos_dir[i] == 1.0:
            # Bullish BOS: find last bearish candle before this bar
            for j in range(i - 1, max(i - bos_n - 1, -1), -1):
                if c_arr[j] < o_arr[j]:  # bearish candle
                    last_bull_ob = (o_arr[j] + c_arr[j]) / 2.0
                    break
        elif bos_dir[i] == -1.0:
            # Bearish BOS: find last bullish candle before this bar
            for j in range(i - 1, max(i - bos_n - 1, -1), -1):
                if c_arr[j] > o_arr[j]:  # bullish candle
        ...
        if not np.isnan(last_bull_ob) and c_arr[i] > 0:
            dists.append(abs(c_arr[i] - last_bull_ob) / c_arr[i])
        if not np.isnan(last_bear_ob) and c_arr[i] > 0:
            dists.append(abs(c_arr[i] - last_bear_ob) / c_arr[i])
        if dists:
            ob_dist[i] = min(dists)
    ...
    for i in range(swing_lookback, n):
        # Find most recent major swing low: bar with the lowest low in prior 50+ bars
        lookback_start = max(0, i - swing_lookback)
        swing_low_idx = lookback_start + np.argmin(l_arr[lookback_start:i + 1])
        swing_high_idx = lookback_start + np.argmax(h_arr[lookback_start:i + 1])

        # Anchored VWAP from swing low
        if swing_low_idx <= i:
            seg_tp = typical_price[swing_low_idx:i + 1]
            seg_v = v_arr[swing_low_idx:i + 1]
            cum_v = np.cumsum(seg_v)
            if cum_v[-1] > 0:
                avwap_low_arr[i] = np.cumsum(seg_tp * seg_v)[-1] / cum_v[-1]

        # Anchored VWAP from swing high
        if swing_high_idx <= i:
            seg_tp = typical_price[swing_high_idx:i + 1]
            seg_v = v_arr[swing_high_idx:i + 1]
            cum_v = np.cumsum(seg_v)
            if cum_v[-1] > 0:
                avwap_high_arr[i] = np.cumsum(seg_tp * seg_v)[-1] / cum_v[-1]

        # Position relative to both AVWAPs
...
def compute_esoteric_features(df: pd.DataFrame, tweets_df: pd.DataFrame,
    ...
    if tweets_df is not None and len(tweets_df) > 0:
        ...
        tw_map = tw_basic.to_dict('index')
        tweets_count = np.full(len(df), np.nan)
        tweet_gold = np.full(len(df), np.nan)
        tweet_red = np.full(len(df), np.nan)
        tweet_green = np.full(len(df), np.nan)
        tweet_dom_color = np.full(len(df), np.nan)
        tweet_color_cnt = np.full(len(df), np.nan)
        tweet_color_sent = np.full(len(df), np.nan)
        for i in range(len(df)):
            bk = bucket_vals[i]
            if bk in tw_map:
                tweets_count[i] = tw_map[bk]['tweets_count']
                tweet_gold[i] = tw_map[bk]['gold_tweet']
                tweet_red[i] = tw_map[bk]['red_tweet']
                tweet_green[i] = tw_map[bk]['green_tweet']
                tweet_dom_color[i] = tw_map[bk]['tweet_dominant_color']
                tweet_color_cnt[i] = tw_map[bk]['tweet_color_count']
                tweet_color_sent[i] = tw_map[bk]['color_sentiment']
    ...
    if news_df is not None and len(news_df) > 0:
        ...
        nw_map = nw_basic.to_dict('index')

        news_count = np.full(len(df), np.nan)
        news_sent = np.full(len(df), np.nan)
        news_caution = np.full(len(df), np.nan)
        for i in range(len(df)):
            bk = bucket_vals[i]
            if bk in nw_map:
                news_count[i] = nw_map[bk].get('news_count', np.nan)
                news_sent[i] = nw_map[bk].get('news_sentiment', np.nan)
                news_caution[i] = nw_map[bk].get('caution_gematria', np.nan)
...
def _compute_planetary_hour_idx(ts_unix_val):
    """Compute planetary hour planet index from a unix timestamp.
    Returns int 0-6 index into PLANET_ORDER, or NaN on failure."""
    try:
        from universal_astro import PLANET_ORDER as _PO
        dt = datetime.utcfromtimestamp(float(ts_unix_val))
        ph = get_planetary_hour(dt)
        return _PO.index(ph['planet'])
    except Exception:
        return np.nan
...
TRIPLE_BARRIER_CONFIG = {
    '5m':  {'tp_atr_mult': 1.5, 'sl_atr_mult': 1.5, 'max_hold_bars': 48},
...
# ============================================================
# TREND-CONTEXT CROSS FEATURES (ALL events x bull/bear regime)
# ============================================================

def _add_trend_cross_features(df, tf_name='1h'):
    """
    Systematically cross ALL event/state features with trend regime.

    For every event that fires, the model needs to know:
    1. Was it in a bull or bear trend? (x_bull / x_bear)
    2. How long ago did it last fire? (bars_since — many already exist)
    3. What did price do after the last occurrence? (return_since)

    Uses HIGHER-TF trend for sub-daily TFs so "bull/bear" reflects the
    real macro trend, not intraday noise:
      5m/15m → h4_trend (8-day context)
      1h     → d_trend  (50-day context)
      4h     → w_trend  (50-week context)
      1d/1w  → same-TF ema50 (already macro)
    """
    close = df.get('close')
    if close is None:
        return
    ...
    if tf_name in ('5m', '15m'):
        htf = df.get('h4_trend')
        if htf is not None:
            bull = pd.to_numeric(htf, errors='coerce')
            bear = (1 - bull)
    elif tf_name == '1h':
        htf = df.get('d_trend')
        if htf is not None:
    ...
    # Fallback to same-TF EMA50 (1d/1w or if HTF not available)
    if bull is None:
        _b = df.get('ema50_rising')
        _br = df.get('ema50_declining')
        if _b is None or _br is None:
            return  # can't compute without any trend context
        bull = pd.to_numeric(_b, errors='coerce')
        bear = pd.to_numeric(_br, errors='coerce')
    ...
    def _safe_col_or(name1, name2):
        """Return first available column as Series, or None."""
        s = _safe_col(name1)
        if s is not None:
            return s
        return _safe_col(name2)

    # Accumulate ALL tx_/ex_/px_ columns in a dict, batch-assign at the end
    # This avoids 300+ individual df[col]=val calls (each triggers DataFrame reindex overhead)
    _tx_batch = {}

    def _add_bull_bear(col_name, signal):
        """Add x_bull and x_bear crosses for a signal."""
        if signal is None:
            return
        s = signal
        cross_name = col_name.replace('west_', '').replace('vedic_', '').replace('cross_', '')
        _tx_batch[f'tx_{cross_name}_x_bull'] = (s * bull).values if hasattr(s * bull, 'values') else np.asarray(s * bull)
        _tx_batch[f'tx_{cross_name}_x_bear'] = (s * bear).values if hasattr(s * bear, 'values') else np.asarray(s * bear)

    def _add_return_since(col_name, signal, threshold=0.5):
        """Add return_since for a binary/flag signal."""
        if signal is None:
            return
        s = signal
        event_mask = s > threshold
        if event_mask.sum() < 5:
            return  # too few events
        cross_name = col_name.replace('west_', '').replace('vedic_', '').replace('cross_', '')
        last_event_price = close.where(event_mask).ffill()
        _tx_batch[f'tx_return_since_{cross_name}'] = ((close - last_event_price) / last_event_price).values

    # ================================================================
    # A) MOON PHASES (already have some from earlier, add missing)
    # ================================================================
    moon = _safe_col('west_moon_phase')
    if moon is not None:
        is_full = ((moon >= 13) & (moon <= 16)).astype(float)
        is_new = ((moon < 2) | (moon > 27.5)).astype(float)
        # These may already exist from the earlier addition — only add if missing
        if 'cross_full_moon_x_bull' not in df.columns:
            _add_bull_bear('full_moon', is_full)
            _add_bull_bear('new_moon', is_new)
            _add_return_since('full_moon', is_full)
            _add_return_since('new_moon', is_new)

    # ================================================================
    # B) MERCURY RETROGRADE
    # ================================================================
    merc = _safe_col('west_mercury_retrograde')
    _add_bull_bear('mercury_retro', merc)
    _add_return_since('mercury_retro', merc)

    # ================================================================
    # C) ECLIPSE WINDOW
    # ================================================================
    eclipse = _safe_col('eclipse_window')
    _add_bull_bear('eclipse', eclipse)
    _add_return_since('eclipse', eclipse)

    # ================================================================
    # D) HARD / SOFT ASPECTS
    # ================================================================
    hard = _safe_col('west_hard_aspects')
    soft = _safe_col('west_soft_aspects')
    if hard is not None:
        hard_high = (hard > hard.rolling(50, min_periods=10).quantile(0.8)).astype(float)
        _add_bull_bear('hard_aspects_high', hard_high)
        _add_return_since('hard_aspects_high', hard_high)
    if soft is not None:
        soft_high = (soft > soft.rolling(50, min_periods=10).quantile(0.8)).astype(float)
        _add_bull_bear('soft_aspects_high', soft_high)
    if hard is not None and soft is not None:
        aspect_ratio = (soft - hard)
        ema_slope = _safe_col('ema50_slope')
        if ema_slope is not None:
            _tx_batch['tx_aspect_ratio_x_trend'] = (aspect_ratio * np.sign(ema_slope)).values

    # ================================================================
    # E) VOID OF COURSE MOON
    # ================================================================
    # VOC moon isn't a direct column — check for it
    voc = _safe_col_or('west_voc_moon', 'voc_moon')
    _add_bull_bear('voc_moon', voc)
    _add_return_since('voc_moon', voc)

    # ================================================================
    # F) VEDIC ASTROLOGY
    # ================================================================
    key_nak = _safe_col('vedic_key_nakshatra')
    _add_bull_bear('key_nakshatra', key_nak)
    _add_return_since('key_nakshatra', key_nak)

    # Nakshatra nature: deva vs rakshasa x trend
    nak_nature = _safe_col('vedic_nature_encoded')
    if nak_nature is not None:
        deva = (nak_nature == 1).astype(float)  # assuming 1=deva
        rakshasa = (nak_nature == 3).astype(float)  # assuming 3=rakshasa
        _add_bull_bear('nakshatra_deva', deva)
        _add_bull_bear('nakshatra_rakshasa', rakshasa)

    # Vedic yoga/tithi x trend
    yoga = _safe_col('vedic_yoga')
    if yoga is not None:
        yoga_change = (yoga != yoga.shift(1)).astype(float)
        _add_bull_bear('yoga_change', yoga_change)

    # ================================================================
    # G) BAZI
    # ================================================================
    bazi_friendly = _safe_col('bazi_btc_friendly')
    _add_bull_bear('bazi_friendly', bazi_friendly)

    bazi_clash = _safe_col('bazi_day_clash_branch')
    _add_bull_bear('bazi_clash', bazi_clash)

    # ================================================================
    # H) ARABIC LOTS
    # ================================================================
    for lot in ['commerce', 'increase', 'catastrophe', 'treachery']:
        lot_conj = _safe_col(f'arabic_lot_{lot}_moon_conj')
        _add_bull_bear(f'arabic_{lot}_conj', lot_conj)

    # ================================================================
    # I) NUMEROLOGY DATES & NUMBERS
    # ================================================================
    # Caution dates
    for date_col in ['day_13', 'day_21', 'is_113', 'is_93', 'is_39', 'is_223', 'is_322']:
        sig = _safe_col(date_col)
        _add_bull_bear(date_col, sig)

    # Pump dates
    for date_col in ['day_27', 'is_37', 'is_73', 'btc_213', 'pump_date']:
        sig = _safe_col(date_col)
        _add_bull_bear(date_col, sig)

    # Price numerology x trend
    master = _safe_col('price_is_master')
    _add_bull_bear('price_master', master)

    vortex = _safe_col('vortex_369')
    _add_bull_bear('vortex_369', vortex)

    # Price contains caution numbers x trend
    for pattern in ['322', '113', '93', '213']:
        sig = _safe_col(f'price_contains_{pattern}')
        _add_bull_bear(f'price_{pattern}', sig)

    # ================================================================
    # J) GEMATRIA MATCHES & FLAGS
    # ================================================================
    gem_caution = _safe_col('tweet_gem_caution')
    _add_bull_bear('gem_caution_tweet', gem_caution)
    _add_return_since('gem_caution_tweet', gem_caution)

    news_caution = _safe_col('news_gem_caution')
    _add_bull_bear('gem_caution_news', news_caution)

    gem_pump = _safe_col('tweet_gem_pump')
    _add_bull_bear('gem_pump_tweet', gem_pump)

    news_pump = _safe_col('news_gem_pump')
    _add_bull_bear('gem_pump_news', news_pump)

    gem_triple = _safe_col('gem_triple_match')
    _add_bull_bear('gem_triple', gem_triple)
    _add_return_since('gem_triple', gem_triple)

    caution_conv = _safe_col('gem_caution_convergence')
    if caution_conv is not None:
        caution_high = (caution_conv >= 2).astype(float)
        _add_bull_bear('gem_caution_convergence', caution_high)
        _add_return_since('gem_caution_convergence', caution_high)

    # ================================================================
    # K) TWEET SIGNALS
    # ================================================================
    gold = _safe_col_or('gold_tweet_today', 'gold_tweet_this_1h')
    _add_bull_bear('gold_tweet', gold)
    _add_return_since('gold_tweet', gold)

    red = _safe_col_or('red_tweet_today', 'red_tweet_this_1h')
    _add_bull_bear('red_tweet', red)
    _add_return_since('red_tweet', red)

    green = _safe_col_or('green_tweet_today', 'green_tweet_this_1h')
    _add_bull_bear('green_tweet', green)

    caps = _safe_col('caps_tweet_decay_fast')
    if caps is not None:
        caps_active = (caps > 0.5).astype(float)
        _add_bull_bear('caps_tweet', caps_active)

    misdirect = _safe_col('misdirection')
    _add_bull_bear('misdirection', misdirect)

    # ================================================================
    # L) SPORTS
    # ================================================================
    upset = _safe_col('sport_upset_count')
    if upset is not None:
        upset_flag = (upset > 0).astype(float)
        _add_bull_bear('sport_upset', upset_flag)
        _add_return_since('sport_upset', upset_flag)

    # ================================================================
    # M) SPACE WEATHER
    # ================================================================
    kp_storm = _safe_col('sw_kp_is_storm')
    _add_bull_bear('kp_storm', kp_storm)
    _add_return_since('kp_storm', kp_storm)

    kp_severe = _safe_col('sw_kp_is_severe')
    _add_bull_bear('kp_severe', kp_severe)

    post_storm = _safe_col('sw_post_storm_7d')
    _add_bull_bear('post_storm_7d', post_storm)

    # ================================================================
    # N) FEAR & GREED EXTREMES
    # ================================================================
    ext_fear = _safe_col('fg_extreme_fear')
    _add_bull_bear('extreme_fear', ext_fear)
    _add_return_since('extreme_fear', ext_fear)

    ext_greed = _safe_col('fg_extreme_greed')
    _add_bull_bear('extreme_greed', ext_greed)
    _add_return_since('extreme_greed', ext_greed)

    # ================================================================
    # O) WYCKOFF EVENTS x TREND
    # ================================================================
    for wk_col in ['wyckoff_spring', 'wyckoff_upthrust', 'wyckoff_sos', 'wyckoff_sow']:
        sig = _safe_col(wk_col)
        _add_bull_bear(wk_col, sig)
        _add_return_since(wk_col, sig)

    wk_score = _safe_col('wyckoff_directional_score')
    if wk_score is not None:
        ema_slope = _safe_col('ema50_slope')
        if ema_slope is not None:
            _tx_batch['tx_wyckoff_score_x_trend'] = (wk_score * np.sign(ema_slope)).values

    # ================================================================
    # P) ICT / SMC x TREND
    # ================================================================
    for ict_col in ['fvg_bullish', 'fvg_bearish', 'liquidity_sweep']:
        sig = _safe_col(ict_col)
        if sig is not None:
            sig_active = (sig != 0).astype(float)
            _add_bull_bear(ict_col, sig_active)

    bos = _safe_col('bos_direction')
    if bos is not None:
        bos_bull = (bos > 0).astype(float)
        bos_bear = (bos < 0).astype(float)
        _tx_batch['tx_bos_bullish_x_bull'] = (bos_bull * bull).values if hasattr(bos_bull * bull, 'values') else np.asarray(bos_bull * bull)
        _tx_batch['tx_bos_bullish_x_bear'] = (bos_bull * bear).values if hasattr(bos_bull * bear, 'values') else np.asarray(bos_bull * bear)
        _tx_batch['tx_bos_bearish_x_bull'] = (bos_bear * bull).values if hasattr(bos_bear * bull, 'values') else np.asarray(bos_bear * bull)
        _tx_batch['tx_bos_bearish_x_bear'] = (bos_bear * bear).values if hasattr(bos_bear * bear, 'values') else np.asarray(bos_bear * bear)

    # ================================================================
    # Q) FUNDING / OI x TREND
    # ================================================================
    fund_regime = _safe_col('funding_regime')
    if fund_regime is not None:
        fund_extreme_pos = (fund_regime >= 2).astype(float)
        fund_extreme_neg = (fund_regime <= -2).astype(float)
        _add_bull_bear('funding_extreme_pos', fund_extreme_pos)
        _add_bull_bear('funding_extreme_neg', fund_extreme_neg)

    oi_div = _safe_col('oi_price_divergence')
    if oi_div is not None:
        _tx_batch['tx_oi_divergence_x_trend'] = (oi_div * bull - oi_div * bear).values

    # ================================================================
    # R) CVD x TREND
    # ================================================================
    cvd_div = _safe_col('cvd_price_divergence')
    if cvd_div is not None:
        cvd_bull_div = (cvd_div > 0).astype(float)  # bullish divergence
        cvd_bear_div = (cvd_div < 0).astype(float)  # bearish divergence
        _add_bull_bear('cvd_bull_divergence', cvd_bull_div)
        _add_bull_bear('cvd_bear_divergence', cvd_bear_div)

    # ================================================================
    # S) CYCLE FEATURES x TREND
    # ================================================================
    for cycle in ['jupiter_365d_sin', 'mercury_1216d_sin', 'schumann_783d_sin',
                  'schumann_133d_sin', 'chakra_heart_161d_sin']:
        sig = _safe_col(cycle)
        if sig is not None:
            cycle_high = (sig > 0.7).astype(float)
            name = cycle.replace('_sin', '_peak')
            _add_bull_bear(name, cycle_high)

    equinox = _safe_col('equinox_proximity')
    if equinox is not None:
        near_equinox = (equinox < 5).astype(float) if equinox.max() > 5 else equinox
        _add_bull_bear('near_equinox', near_equinox)

    # ================================================================
    # T) HEBREW / SHMITA
    # ================================================================
    shmita = _safe_col_or('shmita_year', 'is_shmita')
    _add_bull_bear('shmita', shmita)

    jubilee = _safe_col('jubilee_proximity')
    if jubilee is not None:
        near_jubilee = (jubilee < 30).astype(float) if jubilee.max() > 30 else jubilee
        _add_bull_bear('near_jubilee', near_jubilee)

    # ================================================================
    # U) TIME / SESSION FEATURES
    # ================================================================
    for time_col in ['is_friday', 'is_monday', 'is_weekend', 'is_month_end',
                     'is_quarter_end', 'is_hour_4', 'is_asia_session',
                     'is_london_session', 'is_ny_session']:
        sig = _safe_col(time_col)
        name = time_col.replace('is_', '')
        _add_bull_bear(name, sig)

    # ================================================================
    # V) FIBONACCI / SACRED GEOMETRY
    # ================================================================
    near_f21 = _safe_col('near_fib_21')
    _add_bull_bear('near_fib_21', near_f21)
    _add_return_since('near_fib_21', near_f21)

    near_f13 = _safe_col('near_fib_13')
    _add_bull_bear('near_fib_13', near_f13)
    _add_return_since('near_fib_13', near_f13)

    ema_slope = _safe_col('ema50_slope')
    if ema_slope is not None:
        slope_sign = np.sign(ema_slope)

        gr_dist = _safe_col('golden_ratio_dist')
        if gr_dist is not None:
            _tx_batch['tx_golden_ratio_x_trend'] = (gr_dist * slope_sign).values

        gann_dist = _safe_col('gann_sq9_distance')
        if gann_dist is not None:
            _tx_batch['tx_gann_sq9_x_trend'] = (gann_dist * slope_sign).values

    # ================================================================
    # W) HIGHER-TF RETURNS & TREND
    # ================================================================
    if ema_slope is not None:
        slope_sign = np.sign(ema_slope)
        for htf_col in ['h4_return', 'd_return', 'w_return',
                         'h4_bb_pctb', 'd_bb_pctb', 'w_bb_pctb']:
            sig = _safe_col(htf_col)
            if sig is not None:
                _tx_batch[f'tx_{htf_col}_x_trend'] = (sig * slope_sign).values

    for htf_trend in ['h4_trend', 'd_trend', 'w_trend']:
        sig = _safe_col(htf_trend)
        _add_bull_bear(htf_trend, sig)
    ...
    if ema_slope is not None:
        slope_sign = np.sign(ema_slope)
        for ticker in ['dxy', 'spx', 'vix', 'nasdaq', 'russell', 'hyg', 'tlt',
                        'mstr', 'coin', 'gold', 'oil', 'ibit', 'silver', 'us10y']:
            roc_col = f'macro_{ticker}_roc5d'
            sig = _safe_col(roc_col)
            if sig is not None:
                _tx_batch[f'tx_{roc_col}_x_trend'] = (sig * slope_sign).values

    # ================================================================
    # Y) BB / OVERSOLD COMBOS + OSCILLATORS
    # ================================================================
    for osc_col in ['bb_squeeze_20', 'rsi_14_ob', 'rsi_14_os',
                     'macd_cross_up', 'macd_cross_down', 'volume_spike']:
        sig = _safe_col(osc_col)
        name = osc_col.replace('_20', '').replace('_14_', '_')
        _add_bull_bear(name, sig)

    rsi_div = _safe_col('rsi_bullish_div')
    _add_bull_bear('rsi_bull_div', rsi_div)
    _add_return_since('rsi_bull_div', rsi_div)

    # Consecutive red/green at high threshold (3+)
    cr = _safe_col('consec_red')
    if cr is not None:
        cr_high = (cr >= 3).astype(float)
        _add_bull_bear('consec_red_high', cr_high)
        _add_return_since('consec_red_high', cr_high)

    cg = _safe_col('consec_green')
    if cg is not None:
        cg_high = (cg >= 3).astype(float)
        _add_bull_bear('consec_green_high', cg_high)

    # ================================================================
    # Z) ELLIOTT / TREND INDICATORS
    # ================================================================
    ez = _safe_col('elliott_zigzag')
    if ez is not None:
        elliott_up = (ez > 0).astype(float)
        elliott_down = (ez < 0).astype(float)
        _add_bull_bear('elliott_up', elliott_up)
        _add_bull_bear('elliott_down', elliott_down)

    for trend_sig in ['golden_cross', 'death_cross', 'ichimoku_tk_cross',
                       'supertrend_flip', 'sar_flip']:
        sig = _safe_col(trend_sig)
        _add_bull_bear(trend_sig, sig)

    # ================================================================
    # AA) CANDLE PATTERNS
    # ================================================================
    for candle_col in ['doji', 'bull_engulfing', 'bear_engulfing',
                        'consensio_green_wave', 'consensio_red_wave']:
        sig = _safe_col(candle_col)
        _add_bull_bear(candle_col, sig)

    hmr = _safe_col('hammer')
    _add_bull_bear('hammer', hmr)
    _add_return_since('hammer', hmr)

    ss = _safe_col('shooting_star')
    _add_bull_bear('shooting_star', ss)
    _add_return_since('shooting_star', ss)

    # ================================================================
    # AB) ONCHAIN + DERIVATIVES (advanced)
    # ================================================================
    hash_cap = _safe_col('onchain_hash_rate_capitulation')
    _add_bull_bear('hash_capitulation', hash_cap)
    _add_return_since('hash_capitulation', hash_cap)

    for deriv_col, name in [
        ('coinbase_premium_positive', 'cb_premium_pos'),
        ('funding_rate_neg', 'funding_neg'),
        ('funding_rate_high', 'funding_high'),
        ('post_liq_flag', 'post_liq'),
        ('macro_decorrelation', 'macro_decorr'),
        ('gtrends_interest_high', 'gtrends_high'),
    ]:
        sig = _safe_col(deriv_col)
        _add_bull_bear(name, sig)

    # ================================================================
    # AC) SENTIMENT
    # ================================================================
    for sent_col, name in [
        ('sentiment_alignment', 'sent_alignment'),
        ('sentiment_conflict', 'sent_conflict'),
        ('tweet_gem_btc_energy', 'gem_btc_energy_tweet'),
        ('news_gem_btc_energy', 'gem_btc_energy_news'),
    ]:
        sig = _safe_col(sent_col)
        _add_bull_bear(name, sig)

    # ================================================================
    # AD) HEBREW / CULTURAL CALENDAR
    # ================================================================
    for cal_col in ['yom_kippur_window', 'passover_window', 'rosh_hashanah_window',
                     'sukkot_window', 'chinese_new_year_window', 'diwali_window',
                     'ramadan_window', 'date_palindrome']:
        sig = _safe_col(cal_col)
        name = cal_col.replace('_window', '')
        _add_bull_bear(name, sig)

    # ================================================================
    # AE) MAYAN ASTROLOGY
    # ================================================================
    for mayan_col in ['mayan_tone_1', 'mayan_tone_9', 'mayan_tone_13']:
        sig = _safe_col(mayan_col)
        _add_bull_bear(mayan_col, sig)

    # ================================================================
    # AF) VOLUME PROFILE / AVWAP
    # ================================================================
    vpoc_mig = _safe_col('vpoc_migration')
    if vpoc_mig is not None:
        vpoc_up = (vpoc_mig > 0).astype(float)
        vpoc_down = (vpoc_mig < 0).astype(float)
        _add_bull_bear('vpoc_up', vpoc_up)
        _add_bull_bear('vpoc_down', vpoc_down)

    va_pos = _safe_col('value_area_position')
    if va_pos is not None:
        va_above = (va_pos > 0).astype(float)
        va_below = (va_pos < 0).astype(float)
        _add_bull_bear('va_above', va_above)
        _add_bull_bear('va_below', va_below)

    avwap = _safe_col('avwap_position')
    if avwap is not None:
        avwap_above = (avwap > 0).astype(float)
        avwap_below = (avwap < 0).astype(float)
        _add_bull_bear('avwap_above', avwap_above)
        _add_bull_bear('avwap_below', avwap_below)

    # ================================================================
    # AG) ADDITIONAL SPACE WEATHER
    # ================================================================
    sw_phase = _safe_col('sw_solar_cycle_phase')
    if sw_phase is not None:
        sw_ascending = (sw_phase == 1).astype(float)
        _add_bull_bear('solar_ascending', sw_ascending)

    kp_delta = _safe_col('sw_kp_delta_3d')
    if kp_delta is not None:
        kp_rising = (kp_delta > 1).astype(float)
        _add_bull_bear('kp_rising', kp_rising)

    # ================================================================
    # AH) ADDITIONAL WYCKOFF / ICT (beyond O/P)
    # ================================================================
    wk_evr = _safe_col('wyckoff_effort_vs_result')
    if wk_evr is not None and ema_slope is not None:
        _tx_batch['tx_wyckoff_evr_x_trend'] = (wk_evr * np.sign(ema_slope)).values

    wk_vt = _safe_col('wyckoff_volume_trend')
    if wk_vt is not None:
        wk_vol_up = (wk_vt > 0).astype(float)
        _add_bull_bear('wk_vol_up', wk_vol_up)

    fvg_dist = _safe_col('fvg_nearest_distance')
    if fvg_dist is not None and ema_slope is not None:
        _tx_batch['tx_fvg_dist_x_trend'] = (fvg_dist * np.sign(ema_slope)).values

    ob_dist = _safe_col('order_block_distance')
    if ob_dist is not None and ema_slope is not None:
        _tx_batch['tx_ob_dist_x_trend'] = (ob_dist * np.sign(ema_slope)).values

    # ================================================================
    # AI) MARKET CALENDAR EVENTS
    # ================================================================
    for mkt_col in ['fomc_day', 'fomc_window', 'cpi_window', 'nfp_window',
                     'monthly_opex', 'monthly_opex_window', 'quarterly_opex',
                     'quarterly_opex_window', 'halving_window',
                     'us_tax_window', 'quarterly_tax_window', 'payday_window',
                     'tax_loss_harvest', 'january_effect',
                     'black_friday_window', 'super_bowl_window',
                     'summer_doldrums', 'bonus_season',
                     'election_year', 'election_window', 'midterm_window',
                     'btc_birthday', 'pi_cycle_cross']:
        sig = _safe_col(mkt_col)
        _add_bull_bear(mkt_col, sig)

    # Halving proximity: continuous, use trend interaction
    halv_prox = _safe_col('halving_proximity')
    if halv_prox is not None and ema_slope is not None:
        _tx_batch['tx_halving_proximity_x_trend'] = (halv_prox * np.sign(ema_slope)).values

    # ================================================================
    # AJ) NEW DAY-OF-WEEK/MONTH + NUMEROLOGY FEATURES x TREND
    # ================================================================
    for new_col in ['is_tuesday', 'is_wednesday', 'is_thursday', 'is_sunday',
                    'is_day_5', 'is_day_7', 'is_day_11', 'is_day_12',
                    'is_day_15', 'is_day_17', 'is_day_20', 'is_day_28',
                    'is_48', 'is_84', 'is_43', 'is_34', 'is_11_doy',
                    'is_72', 'is_71', 'is_91', 'is_311', 'is_312', 'is_132',
                    'price_dr_3', 'price_dr_7', 'price_dr_9',
                    'dr_mirror_match', 'dr_completion_9',
                    'price_contains_72', 'price_contains_71', 'price_contains_91',
                    'price_contains_39', 'price_contains_37', 'price_contains_73',
                    'month_day_sum_7', 'month_day_sum_11', 'month_day_sum_13',
                    'month_day_sum_22', 'week_master', 'pct_year_93',
                    'doy_div_9', 'is_fibonacci_day', 'price_near_round_1000',
                    'is_sequential_date', 'doy_power_of_2', 'day_is_prime']:
        sig = _safe_col(new_col)
        _add_bull_bear(new_col, sig)

    # Planetary day resonance x trend (continuous -1/0/1)
    res = _safe_col('planetary_day_resonance')
    if res is not None:
        _tx_batch['tx_planetary_resonance_x_bull'] = (res * bull).values if hasattr(res * bull, 'values') else np.asarray(res * bull)
        _tx_batch['tx_planetary_resonance_x_bear'] = (res * bear).values if hasattr(res * bear, 'values') else np.asarray(res * bear)

    # ================================================================
    # AJ3) DOY 1-365 x TREND (bull/bear) — vectorized
    # ================================================================
    _doy_data = {}
    for d in range(1, 366):
        doy_sig = _safe_col(f'doy_{d}')
        if doy_sig is not None and doy_sig.sum() > 0:
            _doy_data[d] = doy_sig.fillna(0).values.astype(np.float32)

    if _doy_data:
        bull_arr = bull.values.astype(np.float32) if hasattr(bull, 'values') else np.asarray(bull, dtype=np.float32)
        bear_arr = bear.values.astype(np.float32) if hasattr(bear, 'values') else np.asarray(bear, dtype=np.float32)

        for d, ds in _doy_data.items():
            _tx_batch[f'tx_doy_{d}_x_bull'] = ds * bull_arr
            _tx_batch[f'tx_doy_{d}_x_bear'] = ds * bear_arr

    # ================================================================
    # AJ2) POWER CROSSES (px_*) x TREND CONTEXT
    # These are the strongest crosses from cross_test.py analysis.
    # They need bull/bear context so the model knows regime.
    # ================================================================
    # Exclude px_sys_ (already double-crossed from systematic tester — re-crossing would be quadruple)
    _px_cols = [c for c in df.columns if c.startswith('px_') and not c.startswith('px_sys_')]
    for px_col in _px_cols:
        sig = _safe_col(px_col)
        if sig is None:
            continue
        s = sig
        if s.abs().sum() > 0:
            _tx_batch[f'{px_col}_x_bull'] = (s * bull).values if hasattr(s * bull, 'values') else np.asarray(s * bull)
            _tx_batch[f'{px_col}_x_bear'] = (s * bear).values if hasattr(s * bear, 'values') else np.asarray(s * bear)

    # --- Batch flush tx_/px_ columns to df before VWAP/range cross sections ---
    # These sections iterate over existing tx_/px_ columns, so they need to be on df first
    if _tx_batch:
        print(f'    [TREND CROSS] Batch assigning {len(_tx_batch):,} tx_/px_ columns...', flush=True)
        _tx_df = pd.DataFrame(_tx_batch, index=df.index)
        df[_tx_df.columns] = _tx_df
        del _tx_df
        _tx_batch.clear()

    # ================================================================
    # AK) VWAP CROSS DIMENSION — all existing signals x above/below VWAP
    # ================================================================
    avwap_pos = _safe_col('avwap_position')
    ...
    if vwap_signal is not None:
        ...
        _vwap_dict = {}
        for bull_col in _existing_bull:
            base = bull_col.replace('_x_bull', '')
            # Determine prefix for output naming
            prefix = 'tx_' if bull_col.startswith('tx_') else 'px_'
            raw_name = base[len(prefix):]
            bear_col = f'{base}_x_bear'
            if bear_col in df.columns:
                sig_approx = pd.to_numeric(df[bull_col], errors='coerce') + \
                             pd.to_numeric(df[bear_col], errors='coerce')
                if sig_approx.sum() > 0:
                    _vwap_dict[f'{prefix}{raw_name}_x_above_vwap'] = (sig_approx * above_vwap).values
                    _vwap_dict[f'{prefix}{raw_name}_x_below_vwap'] = (sig_approx * below_vwap).values
        if _vwap_dict:
            _vwap_df = pd.DataFrame(_vwap_dict, index=df.index)
            df[_vwap_df.columns] = _vwap_df
            del _vwap_dict, _vwap_df

    # ================================================================
    # AL) RANGE POSITION CROSS DIMENSION — signals x top/bottom of range
    # ================================================================
        ...
        for bull_col in _existing_bull:
            ...
            if bear_col in df.columns:
                sig_approx = pd.to_numeric(df[bull_col], errors='coerce') + \
                             pd.to_numeric(df[bear_col], errors='coerce')
                if sig_approx.sum() > 0:
                    _range_dict[f'{prefix}{raw_name}_x_range_top'] = (sig_approx * range_top).values
                    _range_dict[f'{prefix}{raw_name}_x_range_bottom'] = (sig_approx * range_bottom).values
        if _range_dict:
            _range_df = pd.DataFrame(_range_dict, index=df.index)
            df[_range_df.columns] = _range_df
            del _range_dict, _range_df

    # ================================================================
    # AM) ESOTERIC x RSI/SAR/BB CROSSES (top 25 key signals)
    # ================================================================
    _KEY_ESOTERIC_SIGNALS = [
        'west_moon_phase', 'eclipse_window', 'west_mercury_retrograde',
        'vedic_key_nakshatra', 'vedic_nature_encoded', 'bazi_day_clash_branch',
        'is_37', 'is_73', 'is_93', 'is_113', 'is_17', 'is_19',
        'btc_213', 'pump_date', 'vortex_369',
        'sw_kp_is_storm', 'schumann_133d_sin', 'schumann_783d_sin',
        'fg_extreme_fear', 'fg_extreme_greed',
        'is_day_11', 'is_day_20', 'month_day_sum_7',
        'is_tuesday', 'is_wednesday',
    ] + [f'doy_{d}' for d in range(1, 366)]  # ALL DOY flags x TA regimes

    # RSI crosses
    rsi_ob = _safe_col('rsi_14_ob')
    rsi_os = _safe_col('rsi_14_os')
    # If rsi_14_ob/os don't exist, try to derive from rsi_14
    if rsi_ob is None:
        rsi_raw = _safe_col('rsi_14')
        if rsi_raw is not None:
            rsi_ob = (rsi_raw > 70).astype(float)
            rsi_os = (rsi_raw < 30).astype(float)

    # SAR crosses
    sar_bull = _safe_col('sar_flip')
    sar_raw = _safe_col('sar_direction')
    if sar_raw is not None:
        sar_bull_sig = (sar_raw > 0).astype(float)
        sar_bear_sig = (sar_raw < 0).astype(float)
    elif sar_bull is not None:
        sar_bull_sig = (sar_bull > 0).astype(float)
        sar_bear_sig = (sar_bull < 0).astype(float) if sar_bull is not None else None
    else:
        sar_bull_sig = None
        sar_bear_sig = None

    # BB crosses
    bb_pctb = _safe_col('bb_pctb')
    if bb_pctb is not None:
        bb_above = (bb_pctb > 1.0).astype(float)
        bb_below = (bb_pctb < 0.0).astype(float)
    else:
        bb_above = None
        bb_below = None

    # MACD crosses
    macd_hist = _safe_col('macd_histogram')
    if macd_hist is None:
        macd_hist = _safe_col('macd_hist')
    if macd_hist is not None:
        macd_bull = (macd_hist > 0).astype(float)
        macd_bear = (macd_hist < 0).astype(float)
        macd_cross_up_sig = _safe_col('macd_cross_up')
        macd_cross_down_sig = _safe_col('macd_cross_down')
        macd_bull_fading = ((macd_hist > 0) & (macd_hist < macd_hist.shift(1))).astype(float)
        macd_bear_fading = ((macd_hist < 0) & (macd_hist > macd_hist.shift(1))).astype(float)
    else:
        macd_bull = macd_bear = macd_cross_up_sig = macd_cross_down_sig = None
        macd_bull_fading = macd_bear_fading = None

    # Build the TA regime dict
    _ta_regimes = {}
    if rsi_ob is not None:
        _ta_regimes['rsi_ob'] = rsi_ob
    if rsi_os is not None:
        _ta_regimes['rsi_os'] = rsi_os
    if sar_bull_sig is not None:
        _ta_regimes['sar_bull'] = sar_bull_sig
    if sar_bear_sig is not None:
        _ta_regimes['sar_bear'] = sar_bear_sig
    if bb_above is not None:
        _ta_regimes['bb_above'] = bb_above
    if bb_below is not None:
        _ta_regimes['bb_below'] = bb_below
    if macd_bull is not None:
        _ta_regimes['macd_bull'] = macd_bull
    if macd_bear is not None:
        _ta_regimes['macd_bear'] = macd_bear
    if macd_cross_up_sig is not None:
        _ta_regimes['macd_cross_up'] = macd_cross_up_sig
    if macd_cross_down_sig is not None:
        _ta_regimes['macd_cross_down'] = macd_cross_down_sig
    if macd_bull_fading is not None:
        _ta_regimes['macd_bull_fading'] = macd_bull_fading
    if macd_bear_fading is not None:
        _ta_regimes['macd_bear_fading'] = macd_bear_fading

    # Cross key esoteric signals with TA regimes — accumulate in dict
    _ex_batch = {}
    for eso_name in _KEY_ESOTERIC_SIGNALS:
        eso_sig = _safe_col(eso_name)
        if eso_sig is None:
            continue
        eso_s = eso_sig
        # For continuous signals, make binary (>0.5 threshold)
        if eso_s.max() > 1.5:
            eso_s = (eso_s > eso_s.median()).astype(float)
        clean_name = eso_name.replace('west_', '').replace('vedic_', '').replace('sw_', '')
        eso_vals = eso_s.values if hasattr(eso_s, 'values') else np.asarray(eso_s)
        for regime_name, regime_sig in _ta_regimes.items():
            rs_vals = regime_sig.values if hasattr(regime_sig, 'values') else np.asarray(regime_sig)
            _ex_batch[f'ex_{clean_name}_x_{regime_name}'] = eso_vals * rs_vals

    # --- Batch flush ex_ columns ---
    if _ex_batch:
        print(f'    [ESO x TA] Batch assigning {len(_ex_batch):,} ex_ columns...', flush=True)
        _ex_df = pd.DataFrame(_ex_batch, index=df.index)
        df[_ex_df.columns] = _ex_df
        del _ex_df, _ex_batch

    # ================================================================
    # AM2) DOY x ALL CONTEXTS — everything x everything
    # Every DOY crossed with every base feature (binarized).
    # This is the conditional calendar — "what was happening on this DOY?"
    # SKIP for 1W/1D: too few rows for meaningful DOY crosses (339/2368 rows)
    # ================================================================
    _skip_prefixes = ('tx_', 'px_', 'ex_', 'dx_', 'cross_', 'next_', 'target_', 'doy_')
    _skip_exact = {'timestamp', 'open', 'high', 'low', 'close', 'volume', 'quote_volume',
                   'trades', 'taker_buy_volume', 'taker_buy_quote', 'triple_barrier_label'}

    _ALL_CONTEXTS = {}
    for col_name in df.columns:
        if col_name.startswith(_skip_prefixes) or col_name in _skip_exact:
            continue
        sig = _safe_col(col_name)
        if sig is None:
            continue
        s = sig
        n_unique = s.nunique()
        ...
        if n_unique <= 3:
            # Binary — use directly if it fires enough
            b = (s > 0).astype(float)
            if 50 < b.sum() < len(b) * 0.98:
                _ALL_CONTEXTS[col_name] = b
        else:
            # Continuous — binarize at 80th/20th percentile
            try:
                q80 = s.quantile(0.8)
                q20 = s.quantile(0.2)
                high = (s > q80).astype(float)
                low = (s < q20).astype(float)
                if high.sum() > 50:
                    _ALL_CONTEXTS[f'{col_name}_HIGH'] = high
                if low.sum() > 50:
                    _ALL_CONTEXTS[f'{col_name}_LOW'] = low
            except Exception:
                pass

    # Cross ALL DOY flags with ALL contexts — GPU ACCELERATED
    # Skip for 1W/1D: too few rows for meaningful DOY crosses (339/2368 rows vs 365 DOY flags)
    if tf_name in ('1w', '1d'):
        print(f"  [DOY crosses] SKIPPED for {tf_name} — too few rows ({len(df)}) for 365 DOY flags", flush=True)
        _ALL_CONTEXTS = {}
    if _ALL_CONTEXTS:
        n_created = 0

        # Collect DOY arrays that have data
        doy_names = []
        doy_arrays = []
        for d in range(1, 366):
            doy_sig = _safe_col(f'doy_{d}')
            if doy_sig is None or doy_sig.sum() == 0:
                continue
            doy_names.append(d)
            doy_arrays.append(doy_sig.fillna(0).values.astype(np.float32))

        ctx_names = list(_ALL_CONTEXTS.keys())
        ctx_arrays = [v.values.astype(np.float32) for v in _ALL_CONTEXTS.values()]
        del _ALL_CONTEXTS  # free the dict early

        if doy_arrays and ctx_arrays and _HAS_GPU:
            # GPU batch multiply with CHUNKED contexts to avoid RAM OOM
            # Instead of column_stack(all 10K contexts) = 8.7GB, chunk into groups of 200
            # Adaptive chunk size based on available RAM
            try:
                import psutil
                ...
                try:
                    with open('/proc/meminfo') as f:
                        _ram_gb = int(f.readline().split()[1]) / (1024**2)
                except:
                    _ram_gb = 64
            if _ram_gb >= 512:
                CTX_CHUNK = 2000
                ...
                CTX_CHUNK = 200  # 200 * 217K rows * 4 bytes = ~170MB per chunk
            try:
                doy_np = np.ascontiguousarray(np.column_stack(doy_arrays))   # (N, n_doy) — small
                n_doy = len(doy_names)
                n_ctx = len(ctx_names)
                use_gpus = min(_N_GPUS, max(1, n_doy // 10))

                # Split DOYs across GPUs
                doy_per_gpu = max(1, n_doy // use_gpus)
                gpu_splits = []
                for g in range(use_gpus):
                    g_start = g * doy_per_gpu
                    g_end = min(n_doy, (g + 1) * doy_per_gpu) if g < use_gpus - 1 else n_doy
                    if g_start < n_doy:
                        gpu_splits.append((g, g_start, g_end))

                n_ctx_chunks = (n_ctx + CTX_CHUNK - 1) // CTX_CHUNK
                print(f'    [DOY_x_ALL] {n_doy} DOYs x {n_ctx} contexts ({n_ctx_chunks} ctx chunks of {CTX_CHUNK}) on {len(gpu_splits)} GPUs...', flush=True)

                # Dynamic DOY cross batch based on GPU VRAM
                try:
                    from hardware_detect import detect_hardware, get_min_vram_gb
                    _hw = detect_hardware()
                    _min_vram = get_min_vram_gb(_hw)
                    if _min_vram > 0:
                        BATCH = max(5, int(_min_vram * 0.5 * (1024**3) / max(1, doy_np.shape[0] * 200 * 4)))
                    else:
                        BATCH = 25
                except Exception:
                    BATCH = 25
                _dx_dict = {}  # Accumulate all crosses in dict (batch assign)

                # Process contexts in chunks to avoid RAM explosion
                for ctx_chunk_idx in range(0, n_ctx, CTX_CHUNK):
                    ctx_chunk_end = min(ctx_chunk_idx + CTX_CHUNK, n_ctx)
                    ctx_chunk_names = ctx_names[ctx_chunk_idx:ctx_chunk_end]
                    ctx_chunk_np = np.ascontiguousarray(
                        np.column_stack(ctx_arrays[ctx_chunk_idx:ctx_chunk_end])
                    )  # (N, chunk_size) — ~170MB max

                    for gpu_id, g_start, g_end in gpu_splits:
                        with cp.cuda.Device(gpu_id):
                            ctx_gpu = cp.asarray(ctx_chunk_np)
                            doy_slice = cp.asarray(np.ascontiguousarray(doy_np[:, g_start:g_end]))

                            for b_start in range(0, g_end - g_start, BATCH):
                                b_end = min(b_start + BATCH, g_end - g_start)
                                doy_batch = doy_slice[:, b_start:b_end]

                                # Outer product: (N, batch, 1) * (N, 1, chunk) = (N, batch, chunk)
                                crosses = doy_batch[:, :, None] * ctx_gpu[:, None, :]
                                crosses_np = cp.asnumpy(crosses)
                                del crosses

                                for i_local in range(b_end - b_start):
                                    d = doy_names[g_start + b_start + i_local]
                                    for j, ctx_name in enumerate(ctx_chunk_names):
                                        col_data = crosses_np[:, i_local, j]
                                        if col_data.sum() > 0:
                                            ctx_short = ctx_name[:30]
                                            _dx_dict[f'dx_{d}_x_{ctx_short}'] = col_data
                                            n_created += 1

                            del ctx_gpu, doy_slice
                            cp.get_default_memory_pool().free_all_blocks()

                    del ctx_chunk_np
                    if (ctx_chunk_idx // CTX_CHUNK) % 10 == 0:
                        print(f'    [DOY_x_ALL] ctx chunk {ctx_chunk_idx//CTX_CHUNK + 1}/{n_ctx_chunks}, {n_created:,} crosses so far...', flush=True)

                # Batch assign all dx_ crosses at once
                if _dx_dict:
                    print(f'    [DOY_x_ALL] Batch assigning {len(_dx_dict):,} crosses...', flush=True)
                    _dx_df = pd.DataFrame(_dx_dict, index=df.index)
                    df[_dx_df.columns] = _dx_df
                    del _dx_dict, _dx_df

                print(f'    [DOY_x_ALL {len(gpu_splits)}xGPU] {n_ctx} contexts x {n_doy} DOYs '
                      f'= {n_created:,} crosses created')
            except Exception as gpu_err:
                print(f'    [DOY_x_ALL] GPU FAILED: {gpu_err}', flush=True)
                print(f'    [DOY_x_ALL] Retrying on GPU0 only (single GPU, chunked)...', flush=True)
                try:
                    _dx_dict = {}
                    with cp.cuda.Device(0):
                        # Process contexts in chunks on GPU0 fallback too
                        for ctx_chunk_idx in range(0, n_ctx, CTX_CHUNK):
                            ctx_chunk_end = min(ctx_chunk_idx + CTX_CHUNK, n_ctx)
                            ctx_chunk_names = ctx_names[ctx_chunk_idx:ctx_chunk_end]
                            ctx_chunk_np = np.ascontiguousarray(
                            ...
                            ctx_gpu = cp.asarray(ctx_chunk_np)
                            doy_gpu = cp.asarray(doy_np)

                            for b_start in range(0, n_doy, BATCH):
                                b_end = min(b_start + BATCH, n_doy)
                                doy_batch = doy_gpu[:, b_start:b_end]
                                crosses = doy_batch[:, :, None] * ctx_gpu[:, None, :]
                                crosses_np = cp.asnumpy(crosses)
                                del crosses
                                for i_local in range(b_end - b_start):
                                    d = doy_names[b_start + i_local]
                                    for j, ctx_name in enumerate(ctx_chunk_names):
                                        col_data = crosses_np[:, i_local, j]
                                        if col_data.sum() > 0:
                                            ctx_short = ctx_name[:30]
                            ...
                            del ctx_gpu, doy_gpu, ctx_chunk_np
                            cp.get_default_memory_pool().free_all_blocks()

                    # Batch assign
                    if _dx_dict:
                        print(f'    [DOY_x_ALL GPU0] Batch assigning {len(_dx_dict):,} crosses...', flush=True)
                        _dx_df = pd.DataFrame(_dx_dict, index=df.index)
                        df[_dx_df.columns] = _dx_df
                        del _dx_dict, _dx_df
                    print(f'    [DOY_x_ALL GPU0] {n_ctx} contexts x {n_doy} DOYs = {n_created:,} crosses created')
                except Exception as gpu_err2:
                    print(f'    [DOY_x_ALL] CRITICAL: GPU0 also failed: {gpu_err2}', flush=True)
                    raise RuntimeError(f'DOY cross generation failed on GPU: {gpu_err2}')
        elif doy_arrays and ctx_arrays:
            # CPU path (no GPU) — batch assign, process in chunks to limit RAM
            _dx_dict = {}
            for d_idx, d in enumerate(doy_names):
                ds = doy_arrays[d_idx]
                for ctx_name, ctx_arr in zip(ctx_names, ctx_arrays):
                    ctx_short = ctx_name[:30]
                    result = ds * ctx_arr
                    if result.sum() > 0:
                        _dx_dict[f'dx_{d}_x_{ctx_short}'] = result
                        n_created += 1
            if _dx_dict:
                _dx_df = pd.DataFrame(_dx_dict, index=df.index)
                df[_dx_df.columns] = _dx_df
                del _dx_dict, _dx_df
            print(f'    [DOY_x_ALL] {len(ctx_names)} contexts x {len(doy_names)} DOYs '
                  f'= {n_created:,} crosses created')

    # NO dx_ support filter — LightGBM decides via tree splits + min_data_in_leaf.
    # Esoteric calendar signals ARE sparse. That's the edge, not noise.

    # ================================================================
    # AN) ADVANCED TA CROSSES (Ichimoku/Wyckoff/Consensio/Supertrend/Gann)
    # ================================================================
    # Ichimoku cloud position
    kumo_pos = _safe_col('ichimoku_cloud_position')
    if kumo_pos is None:
        kumo_pos = _safe_col('above_kumo')
    if kumo_pos is not None:
        above_kumo = (kumo_pos > 0).astype(float)
        below_kumo = (kumo_pos <= 0).astype(float)
    else:
        above_kumo = below_kumo = None

    # Wyckoff spring/upthrust
    wk_spring = _safe_col('wyckoff_spring')
    wk_upthrust = _safe_col('wyckoff_upthrust')

    # BB squeeze
    bb_squeeze = _safe_col('bb_squeeze_20')
    if bb_squeeze is None:
        bb_squeeze = _safe_col('bb_squeeze')

    # Consensio waves
    green_wave = _safe_col('consensio_green_wave')
    red_wave = _safe_col('consensio_red_wave')

    # Supertrend
    st_flip = _safe_col('supertrend_flip')
    if st_flip is not None:
        st_bull = (st_flip > 0).astype(float)
        st_bear = (st_flip < 0).astype(float)
    else:
        st_bull = st_bear = None

    # CVD divergence
    cvd_div_sig = _safe_col('cvd_price_divergence')

    # Doji
    doji_sig = _safe_col('doji')

    # Gann sq9 proximity
    gann_near = _safe_col('near_gann_sq9')
    if gann_near is None:
        gann_dist = _safe_col('gann_sq9_distance')
        if gann_dist is not None:
            gann_near = (gann_dist.abs() < gann_dist.abs().quantile(0.2)).astype(float)

    # AVWAP position
    avwap_above_sig = above_vwap if vwap_signal is not None else None
    avwap_below_sig = below_vwap if vwap_signal is not None else None

    _adv_ta = {}
    if above_kumo is not None:
        _adv_ta['above_kumo'] = above_kumo
        _adv_ta['below_kumo'] = below_kumo
    if wk_spring is not None:
        _adv_ta['wyckoff_spring'] = wk_spring
    if wk_upthrust is not None:
        _adv_ta['wyckoff_upthrust'] = wk_upthrust
    if bb_squeeze is not None:
        _adv_ta['bb_squeeze'] = bb_squeeze
    if green_wave is not None:
        _adv_ta['green_wave'] = green_wave
    if red_wave is not None:
        _adv_ta['red_wave'] = red_wave
    if st_bull is not None:
        _adv_ta['supertrend_bull'] = st_bull
        _adv_ta['supertrend_bear'] = st_bear
    if cvd_div_sig is not None:
        _adv_ta['cvd_divergence'] = (cvd_div_sig != 0).astype(float)
    if doji_sig is not None:
        _adv_ta['doji'] = doji_sig
    if gann_near is not None:
        _adv_ta['near_gann_sq9'] = gann_near
    if avwap_above_sig is not None:
        _adv_ta['above_avwap'] = avwap_above_sig
        _adv_ta['below_avwap'] = avwap_below_sig

    # Cross top esoteric signals with advanced TA (limited set to control count)
    _KEY_ESO_FOR_ADV_TA = [
        'is_37', 'is_73', 'is_93', 'is_113', 'is_17', 'is_19',
        'btc_213', 'pump_date', 'eclipse_window', 'sw_kp_is_storm',
        'vedic_key_nakshatra', 'bazi_day_clash_branch',
        'west_mercury_retrograde', 'vedic_nature_encoded',
        'fg_extreme_fear', 'fg_extreme_greed',
        'is_day_11', 'is_day_20', 'month_day_sum_7',
        'is_tuesday', 'is_wednesday', 'vortex_369',
        'schumann_133d_sin', 'schumann_783d_sin', 'west_moon_phase',
    ]
    _ex_adv_batch = {}
    for eso_name in _KEY_ESO_FOR_ADV_TA:
        eso_sig = _safe_col(eso_name)
        if eso_sig is None:
            continue
        eso_s = eso_sig
        clean_name = eso_name.replace('west_', '').replace('vedic_', '').replace('sw_', '')
        eso_vals = eso_s.values if hasattr(eso_s, 'values') else np.asarray(eso_s)
        for ta_name, ta_sig in _adv_ta.items():
            ts = ta_sig if ta_sig is not None else None
            if ts is not None:
                ts_vals = ts.values if hasattr(ts, 'values') else np.asarray(ts)
                _ex_adv_batch[f'ex_{clean_name}_x_{ta_name}'] = eso_vals * ts_vals

    # --- Batch flush advanced TA crosses ---
    if _ex_adv_batch:
        print(f'    [ESO x ADV_TA] Batch assigning {len(_ex_adv_batch):,} ex_ columns...', flush=True)
        _ex_adv_df = pd.DataFrame(_ex_adv_batch, index=df.index)
        df[_ex_adv_df.columns] = _ex_adv_df
        del _ex_adv_df, _ex_adv_batch
...
def build_all_features(ohlcv: pd.DataFrame, esoteric_frames: dict,
    ...
    # 13b. KNN x trend cross (uses same HTF-aware regime as _add_trend_cross_features)
