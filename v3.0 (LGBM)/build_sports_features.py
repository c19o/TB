...
if all_games:
    ...
    for side in ['home_team', 'away_team', 'winner']:
        games_df[f'{side}_ordinal'] = games_df[side].apply(ordinal_gematria)
        games_df[f'{side}_dr'] = games_df[side].apply(lambda x: digital_root(ordinal_gematria(x)))
        games_df[f'{side}_reverse'] = games_df[side].apply(reverse_ordinal)

    # Score gematria
    games_df['total_score_dr'] = games_df['total_score'].apply(digital_root)
    games_df['score_diff'] = (games_df['home_score'] - games_df['away_score']).abs()
    games_df['score_diff_dr'] = games_df['score_diff'].apply(digital_root)

    # Score contains special numbers
    for num in ['33', '22', '11', '13', '27', '37', '93', '39']:
        games_df[f'score_contains_{num}'] = games_df['total_score'].apply(
            lambda x: 1 if num in str(x) else 0)
...
daily['game_date_dr'] = daily['date_dt'].apply(
    lambda d: digital_root(sum(int(x) for x in d.strftime('%Y%m%d'))))
