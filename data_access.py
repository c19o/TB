...
class OfflineDataLoader:
    ...
    def load_sports(self, start_ts: int = None) -> dict:
        ...
        games = _safe_read_sql(conn, """
            SELECT date, game_timestamp, winner, home_team, away_team, home_score, away_score,
    ...
    def load_market_signals(self) -> dict:
        """Load DeFi TVL, BTC dominance, mining stats from v2_signals.db."""
        conn = _connect('v2_signals.db', self.db_dir)
        if conn is None:
            return {'defi_tvl': pd.DataFrame(), 'btc_dominance': pd.DataFrame(), 'mining_stats': pd.DataFrame()}

        defi = _safe_read_sql(conn, "SELECT date, total_tvl, total_tvl_change_1d FROM defi_tvl ORDER BY date")
        btcd = _safe_read_sql(conn, """
            SELECT date, btc_dominance, eth_dominance, total_market_cap, total_volume, active_cryptos
            FROM btc_dominance ORDER BY date""")
        mining = _safe_read_sql(conn, """
            SELECT date, hash_rate, difficulty, blocks_mined, miners_revenue, avg_block_size
            FROM mining_stats ORDER BY date""")
        conn.close()

        for frame in [defi, btcd, mining]:
            if not frame.empty:
        ...
        return {'defi_tvl': defi, 'btc_dominance': btcd, 'mining_stats': mining}
...
class LiveDataLoader:
    ...
    def _refresh_sports(self):
        ...
        # Try full column set matching offline loader first
        games = _safe_read_sql(conn, f"""
            SELECT date, game_timestamp, home_team, away_team, winner, venue,
            ...
            ORDER BY date
        """)
        # Fallback: some columns may not exist in the table
        if games.empty:
            games = _safe_read_sql(conn, f"""
        ...
        # Try full horse_races column set matching offline loader
    ...
    def _refresh_space_weather(self):
        ...
        conn = _connect('space_weather.db', self.db_dir)
        if conn is None:
            return
    ...
    def _refresh_market_signals(self):
        conn = _connect('v2_signals.db', self.db_dir)
        if conn is None:
            return
        defi = _safe_read_sql(conn, "SELECT date, total_tvl, total_tvl_change_1d FROM defi_tvl ORDER BY date")
        btcd = _safe_read_sql(conn, "SELECT date, btc_dominance, eth_dominance, total_market_cap, total_volume, active_cryptos FROM btc_dominance ORDER BY date")
        mining = _safe_read_sql(conn, "SELECT date, hash_rate, difficulty, blocks_mined, miners_revenue, avg_block_size FROM mining_stats ORDER BY date")
        conn.close()
        for frame in [defi, btcd, mining]:
            if not frame.empty:
                frame['date'] = pd.to_datetime(frame['date'], errors='coerce')
                ...
                frame.set_index('date', inplace=True)
                frame.sort_index(inplace=True)
        self._caches['market_signals'] = {'defi_tvl': defi, 'btc_dominance': btcd, 'mining_stats': mining}

    def _refresh_astro_cache(self):
        """Refresh daily astro/ephemeris data (called less often)."""
        loader = OfflineDataLoader(self.db_dir)
    ...
    def get_market_signals(self) -> dict:
        return self._caches.get('market_signals', {'defi_tvl': pd.DataFrame(), 'btc_dominance': pd.DataFrame(), 'mining_stats': pd.DataFrame()})
    ...
    def initial_load(self):
        ...
        if conn:
            # Try full column set matching offline loader first
            games = _safe_read_sql(conn, f"""
                SELECT date, home_team, away_team, winner, venue,
                       home_score, away_score, score_total, score_diff,
                       winner_gem_ordinal, winner_gem_dr, score_dr,
                       score_total_dr, is_upset, is_overtime
