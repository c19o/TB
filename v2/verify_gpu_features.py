...
def load_space_weather():
    ...
    # Fallback: kp_history.txt
    kp_path = os.path.join(DB_DIR, 'kp_history.txt')
    ...
    try:
        ...
        print(f"  WARNING: failed to read kp_history.txt: {e}")
        return None

    if rows:
        df = pd.DataFrame(rows).set_index('date')
        print(f"  Loaded kp_history.txt: {len(df)} rows")
        return df
    return None
