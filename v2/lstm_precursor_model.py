...
Output: probability scores for each event type, fed as features to XGBoost.
...
def find_events(df, event_col, condition_fn):
    """Find indices where an event fires."""
    if event_col not in df.columns:
        return np.array([], dtype=int)
    vals = df[event_col].values
    mask = np.zeros(len(vals), dtype=bool)
    for i in range(len(vals)):
        try:
            if not np.isnan(vals[i]) and condition_fn(vals[i]):
                mask[i] = True
