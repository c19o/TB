...
def load_smoke_data(tf_name='1h', max_rows=1000):
    ...
    # 3. Fallback: raw OHLCV from btc_prices.db (will need feature building)
...
def load_smoke_esoteric():
    """Load minimal esoteric data for smoke testing."""
    frames = {}

    # Tweets — last 500
    tw_path = os.path.join(PROJECT_DIR, 'tweets.db')
    if os.path.exists(tw_path):
        try:
            conn = sqlite3.connect(tw_path)
            tw = pd.read_sql_query("SELECT * FROM tweets ORDER BY ts_unix DESC LIMIT 500", conn)
            conn.close()
            if len(tw) > 0:
                frames['tweets'] = tw
                smoke_log(f"Tweets: {len(tw)} rows")
        except Exception as e:
            smoke_log(f"Tweets load failed: {e}")

    # News — last 500
    nw_path = os.path.join(PROJECT_DIR, 'news_articles.db')
    if os.path.exists(nw_path):
        try:
            conn = sqlite3.connect(nw_path)
            nw = pd.read_sql_query("SELECT * FROM articles ORDER BY ts_unix DESC LIMIT 500", conn)
            conn.close()
            if len(nw) > 0:
                frames['news'] = nw
                smoke_log(f"News: {len(nw)} rows")
        except Exception as e:
            smoke_log(f"News load failed: {e}")

    # Sports — last 200
    sp_path = os.path.join(PROJECT_DIR, 'sports_results.db')
    if os.path.exists(sp_path):
        try:
            conn = sqlite3.connect(sp_path)
            for tbl in ['games', 'horse_races']:
                try:
                    sp = pd.read_sql_query(f"SELECT * FROM {tbl} ORDER BY date DESC LIMIT 200", conn)
                    if len(sp) > 0:
                        if 'sports' not in frames:
                            frames['sports'] = {}
                        frames['sports'][tbl] = sp
                        smoke_log(f"Sports/{tbl}: {len(sp)} rows")
                except Exception:
                    pass
            conn.close()
        except Exception:
...
def run_smoke_test(tf_name='1h', max_rows=None):
    ...
    # ============================================================
    # STEP 6: XGBoost training on one CPCV path
    # ============================================================
    try:
        ...
        if len(X_train) < 30 or len(X_test) < 10:
            ...
            try:
                _test_dmat = xgb.DMatrix(np.random.rand(5, 3), label=np.zeros(5))
                xgb.train({'tree_method': 'hist', 'device': 'cuda', 'max_depth': 2}, _test_dmat, num_boost_round=1)
                params['device'] = 'cuda'
                smoke_log("XGBoost GPU: enabled")
            except Exception:
    ...
    # ============================================================
    try:
        # Verify Kelly formula produces sane output
        confidence = 0.65
        rr = 2.0
        base_risk = 0.01
        kelly_frac = 0.25
        ...
        kelly_risk = base_risk * (1 + kelly_frac * kelly_f)
        kelly_risk = min(kelly_risk, base_risk * 3)

        # Drawdown scaling
        for dd in [0.0, 0.05, 0.10, 0.14, 0.15]:
            dd_scale = max(0.0, 1.0 - 2.0 * dd) if dd < 0.15 else 0.0
            final_risk = kelly_risk * dd_scale

        smoke_log(f"Kelly: f={kelly_f:.3f}, risk={kelly_risk:.4f}, halts at 15% DD")
    ...
    # ============================================================
    try:
        from lstm_sequence_model import calibrate_lstm_platt, blend_predictions
