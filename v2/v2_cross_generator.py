...
def _get_gpu_vram_gb():
    ...
    try:
        import cupy as cp
        mem = cp.cuda.Device(0).mem_info  # returns (free, total)
        return mem[1] / (1024**3)  # total VRAM in GB
    except Exception:
        return 12.0  # default: assume 3090


def _get_optimal_batch(n_bars, n_right, gpu_vram_gb=None):
...
def binarize_contexts(df, four_tier=True):
    ...
    for col in df.columns:
        ...
        # NaN preserved — XGBoost treats missing as unknown split direction
...
def gpu_batch_cross(left_names, left_arrays, right_names, right_arrays, prefix,
                    gpu_id=0, min_nonzero=3, max_features=None, col_offset=0):
    """
...
def _gpu_cross_chunk(left_names, left_mat, right_names, right_mat, prefix,
    ...
    gpu_vram_gb = _get_gpu_vram_gb()
    BATCH = _get_optimal_batch(n_bars, n_right, gpu_vram_gb)
        ...
        for i in range(b_end - b_start):
            ...
            for j in range(len(right_names)):
                if sums[i, j] >= min_nonzero:
                    rn = right_names[j]
                    fname = f'{prefix}_{ln[:20]}_{rn[:20]}'
...
def cpu_batch_cross(left_names, left_arrays, right_names, right_arrays, prefix,
                    min_nonzero=3, max_features=None, col_offset=0):
    """CPU fallback — delegates to the chunked gpu_batch_cross with GPU=False path."""
    # Use the unified path which handles chunking
    return gpu_batch_cross(left_names, left_arrays, right_names, right_arrays, prefix,
                           gpu_id=0, min_nonzero=min_nonzero, max_features=max_features,
...
def create_multi_signal_combos(signals, prefix, max_pairs=100):
    ...
    for i in range(n):
        ...
        valid_mask = sums >= 2  # At least 2 co-occurrences
...
def generate_all_crosses(df, tf='1d', gpu_id=0, save_sparse=False, output_dir=None):
    """
    ...
    if len(groups['astro']) >= 2 and groups['ta']:
    ...
    # ── Cross 5: Esoteric × TA ──
    if groups['esoteric'] and groups['ta']:
    ...
    if total_crosses > 0 and all_rows:
        ...
        log(f"  Building COO → CSR ({N:,} × {col_offset:,}), concatenating triplets...")
