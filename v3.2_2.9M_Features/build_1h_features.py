...
=====================
Build 1H feature matrix from BTC/USDT 1H candles using the unified
...
# ============================================================
print("=" * 70)
print("BUILD 1H FEATURE MATRIX (unified pipeline)")
print("=" * 70)

# --- 1H BTC candles (primary OHLCV) ---
print(f"\n{elapsed()} Loading 1H BTC/USDT candles...")
...
btc = pd.read_sql_query("""
    ...
    FROM ohlcv WHERE timeframe='1h' AND symbol='BTC/USDT'
    ORDER BY open_time
""", conn)
conn.close()
...
print(f"  1H candles: {len(btc)} ({btc.index.min()} to {btc.index.max()})")
...
for frame, name in [(btc_4h, '4H'), (btc_1d, '1D'), (btc_1w, '1W')]:
...
# ============================================================
print("=" * 70)
print("Computing 1H features via feature_library.build_all_features()...")
print("=" * 70)
...
print(f"{elapsed()} Calling build_all_features(tf_name='1h', mode='backfill')...")
...
# ============================================================
# GCP (Global Consciousness Project) FEATURES
# ============================================================
print(f"\n{elapsed()} Adding GCP features...")
try:
    from gcp_feature_builder import build_gcp_features
    gcp_feats = build_gcp_features(df)
    for col in gcp_feats.columns:
        df[col] = gcp_feats[col]
    print(f"  GCP features added: {len(gcp_feats.columns)} columns")

    # GCP x trend crosses
    import pandas as _pd
    d_trend = _pd.to_numeric(df.get('d_trend'), errors='coerce')
    bull = d_trend
    bear = 1 - d_trend
    for gcp_col in ['gcp_deviation_mean', 'gcp_deviation_max', 'gcp_rate_of_change', 'gcp_extreme']:
        if gcp_col in df.columns:
            sig = _pd.to_numeric(df[gcp_col], errors='coerce')
            df[f'tx_{gcp_col}_x_bull'] = sig * bull
            df[f'tx_{gcp_col}_x_bear'] = sig * bear

    # GCP x fear/greed cross (the Holmberg paper interaction)
    if 'gcp_deviation_mean' in df.columns:
        gcp_dev = _pd.to_numeric(df['gcp_deviation_mean'], errors='coerce')
        fg_fear = _pd.to_numeric(df.get('fg_extreme_fear'), errors='coerce')
        fg_greed = _pd.to_numeric(df.get('fg_extreme_greed'), errors='coerce')
        df['gcp_x_fear'] = gcp_dev.abs() * fg_fear
        df['gcp_x_greed'] = gcp_dev.abs() * fg_greed
        # GCP x moon phase
        moon = _pd.to_numeric(df.get('west_moon_phase'), errors='coerce')
        is_full = ((moon >= 13) & (moon <= 16)).astype(float)
        is_new = ((moon < 2) | (moon > 27.5)).astype(float)
        df['gcp_x_full_moon'] = gcp_dev.abs() * is_full
        df['gcp_x_new_moon'] = gcp_dev.abs() * is_new
        # GCP x Kp storm
        kp = _pd.to_numeric(df.get('sw_kp_is_storm'), errors='coerce')
        df['gcp_x_kp_storm'] = gcp_dev.abs() * kp

    gcp_cross_count = len([c for c in df.columns if 'gcp_x_' in c or 'tx_gcp_' in c])
    print(f"  GCP cross features: {gcp_cross_count}")
...
# ============================================================
# SAVE TO features_1h.db
# ============================================================
print(f"\n{elapsed()} Saving to features_1h.db...")
...
parquet_path = f'{DB_DIR}/features_1h.parquet'
...
try:
    out_db = f'{DB_DIR}/features_1h.db'
    conn_out = sqlite3.connect(out_db)
    df_save.to_sql('features_1h', conn_out, if_exists='replace', index=False)
    conn_out.execute("CREATE INDEX IF NOT EXISTS idx_1h_ts ON features_1h(timestamp)")
    conn_out.close()
except Exception as e:
...
print(f"\n{'='*70}")
print(f"1H FEATURE MATRIX COMPLETE")
print(f"{'='*70}")
