...
def download_crypto_hourly(conn, symbol, info):
    ...
    Only fetches last ~2 years (enough for intraday training).
    """
    fsym = info['fsym']
    timeframe = '1h'
    ...
    if last_date:
        ...
        # Hourly data: go back ~2 years max (CryptoCompare free limit)
...
def download_all():
    ...
    # ── Crypto hourly (CryptoCompare) ──
    log("\n=== CRYPTO HOURLY (CryptoCompare — 1H, last 2 years) ===")
    crypto_hourly = 0
    for symbol, info in TICKERS_CRYPTO.items():
