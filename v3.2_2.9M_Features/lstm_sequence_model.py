...
def prepare_data(tf_name):
    ...
    # Convert to numpy — impute NaN to 0 (z-scored mean) for LSTM
    # LightGBM handles NaN natively (learns split direction), but LSTM propagates NaN → crash
    X_arr = np.nan_to_num(X_norm.values.astype(np.float32), nan=0.0, posinf=0.0, neginf=0.0)
