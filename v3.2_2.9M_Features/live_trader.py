...
def check_stale_data(tf_name, ohlcv):
    ...
    if hasattr(last_bar_ts, 'tz') and last_bar_ts.tz is not None:
        now = datetime.now(timezone.utc)
    else:
        now = datetime.utcnow()
...
def run_trading_loop(mode='paper'):
    ...
    for tf in ALL_TFS:
        ...
        if os.path.exists(model_path) and feat_path:
            ...
            print(f"  Loaded {tf} model ({len(features_list[tf])} features, {model_age_days:.0f}d old) from {os.path.basename(feat_path)}{age_warning}")
    ...
    while True:
        try:
            ...
            for tf in ALL_TFS:
                ...
                regime, regime_idx = detect_regime(price, sma100, sma100_slope, feat_dict=feat_dict)
                ...
                for trade in open_trades:
                    ...
                    if sl_hit or tp_hit or time_exit:
                        ...
                        conn.execute("""UPDATE trades SET exit_price=?, exit_time=?, pnl=?, pnl_pct=?,
                            ...
                            (slipped_exit, now.isoformat(), pnl, pnl_pct, bars_held, reason, tid))
