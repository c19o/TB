...
# Full CPCV groups per TF (production values from ml_multi_tf.py)
FULL_CPCV_GROUPS = {
    '1w': (4, 1),  # (n_groups, n_test_groups)
    '1d': (4, 1),
    '4h': (5, 2),
    '1h': (6, 2),
    '15m': (6, 2),
...
def load_tf_data(tf_name):
    ...
    if cross_matrix is not None and cross_matrix.shape[0] == X_base.shape[0]:
        ...
        is_sparse = False  # Now dense
        log.info(f"  Combined: {X_all.shape[1]:,} features ({n_base} base + {len(cross_cols):,} crosses) [DENSE]")
...
def build_objective(X_all, y, sample_weights, feature_cols, is_sparse, tf_name,
    ...
    def objective(trial):
        ...
        if narrow_ranges:
            ...
            feature_fraction = trial.suggest_float('feature_fraction', nr['feature_fraction'][0], nr['feature_fraction'][1])
            ...
            max_bin = trial.suggest_categorical('max_bin', nr.get('max_bin', [15, 31, 63]))
        else:
            # Stage 1: wide ranges
            num_leaves = trial.suggest_int('num_leaves', 15, 127)
            min_data_in_leaf = trial.suggest_int('min_data_in_leaf', 1, 30)
            feature_fraction = trial.suggest_float('feature_fraction', 0.01, 0.15)
            ...
            max_bin = trial.suggest_categorical('max_bin', [15, 31, 63])
        ...
        params = {
            ...
            'verbosity': -1,
            'num_threads': max(1, multiprocessing.cpu_count() // max(1, int(os.environ.get('OPTUNA_N_JOBS', '1')))),
        ...
        for fold_i, (train_rel, test_rel) in enumerate(splits):
            ...
            dtrain = lgb.Dataset(X_train_es, label=y_train_es, weight=w_train_es,
                                 free_raw_data=True)
            dval = lgb.Dataset(X_val_es, label=y_val_es, reference=dtrain,
                               free_raw_data=True)  # reference=dtrain reuses bin boundaries
...
def final_retrain(X_all, y, sample_weights, feature_cols, is_sparse,
    ...
    n_groups, n_test_groups = FULL_CPCV_GROUPS.get(tf_name, (6, 2))
    valid_mask = ~np.isnan(y)
    valid_indices = np.where(valid_mask)[0]
...
def run_search_for_tf(tf_name, max_stage=2, n_jobs=1):
    ...
    objective_s1 = build_objective(
        ...
        n_test_groups=1,
        row_subsample=OPTUNA_SEARCH_ROW_SUBSAMPLE,
    ...
    n_groups_full, n_test_full = FULL_CPCV_GROUPS.get(tf_name, (6, 2))
...
def main():
    ...
    if args.n_jobs is not None:
        ...
        # Each LightGBM trial needs threads; balance parallelism vs per-trial speed
        # With 24 cores: 4 trials x 6 threads each
        threads_per_trial = max(4, total_cores // 4)
        n_jobs = max(1, total_cores // threads_per_trial)
